...
{
  die("You must launch Sift command line with the \$sf_sift_lib_dir setting.\n");
}
...
if (file_exists('config/config.php') && !isset($sf_sift_lib_dir))
{
  // allow only upgrading
  if (!in_array('upgrade', $argv))
  {
    echo "Please upgrade your project before launching any other task\n";
    exit();
  }
}

require_once($sf_sift_lib_dir.'/vendor/pake/pakeFunction.php');
require_once($sf_sift_lib_dir.'/vendor/pake/pakeGetopt.class.php');
...
class simpleAutoloader
{
  ...
  static public function initialize($sf_sift_lib_dir)
  {
    self::$class_paths = array();

    self::register($sf_sift_lib_dir, '.class.php');    
...
function __autoload($class)
{
  ...
  if (!$initialized)
  {
    simpleAutoloader::initialize(sfConfig::get('sf_sift_lib_dir'));
    $initialized = true;
  }
...
{
  printf("Sift version %s\n", pakeColor::colorize(trim(file_get_contents($sf_sift_lib_dir.'/VERSION')), 'INFO'));
  exit(0);
}
...
// directory layout
include($sf_sift_data_dir.'/config/constants.php');
...
// register tasks
$dirs = array(  
  sfConfig::get('sf_data_dir').DIRECTORY_SEPARATOR.'tasks'         => '*.php',       // project tasks    
  sfConfig::get('sf_sift_data_dir').DIRECTORY_SEPARATOR.'tasks' => 'sfPake*.php', // sift tasks
