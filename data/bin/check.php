...
check(
    version_compare(phpversion(), '5.2.3', '>='),
    sprintf('PHP version is at least 5.2.3 (Current: %s)', phpversion()),
    'Current version is ' . phpversion(),
    true
);
...
check(
    ...
    'Install the php-xml module (required by Propel)',
    false
);
check(
...
check(
    ...
    'Install token_get_all() function (highly recommended)',
    false
);
...
check(
    ...
    'GD graphics library is installed',
    'Install GD graphics library',
    true
);
check(
    extension_loaded('mcrypt') && function_exists('mcrypt_module_open'),
    'MCrypt module is installed',
    'Install Mcrypt module',
    true
);
check(
    extension_loaded('intl') && class_exists('Collator'),
    'Intl module is installed',
    'Install intl module (Locale comparison methods like sorting may not work ok.)',
    false
);
...
check(
    version_compare(phpversion(), '5.2.9', '!='),
