...
c(this),i=e.data("confirm")||__("Are you sure?");a.confirm('<i class="icon-exclamation-sign icon-large"></i> '+i,function(){var a=e.prop("target");a?b.open(e.attr("href"),a):b.location=e.attr("href")});d.preventDefault()})};a.setupExternalLinks=function(a){c("a.external",a).click(function(){c(this).prop("target","external")})};a.behaviors.coreSetupLinks=function(b){a.setupConfirmLinks(b);a.setupExternalLinks(b)};a.detectTimezone=function(){if(!Cookie.get("timezone_offset")){var a=new Date,b=new Date(a.getFullYear(),
...
else{var j=!1;try{j=c.parseJSON(k.responseText)}catch(u){j=!1}k.status==500?j&&j.html&&a.alert(j.html):k.status==401&&(j&&j.html?j.redirect?a.confirm(j.html,function(){b.location.href=j.redirect}):a.alert(j.html):(i=Config.get("login_url"))?a.confirm(__("This action requires to be logged in and you are not logged in. Redirect to login page?"),function(){b.location.href=i}):a.alert(__("This action requires to be logged in and you are not logged in. Please login.")))}})};a.setup=function(){a.jsEnabled&&
