...
/**
 * Cache response extension for jQuery ajax function.
 *
 * The extensions caches the response of  Ajax requests.
 * Subsequent Ajax requests that match the original Ajax options will return
 * the cached response instead of making a new call.
 * Cached responses can be stored in a RequestStorage storage
 ...
 * @function
 * @author Adam Ayres
 * @link http://github.com/adamayres/jqueryplugins/tree/master/ajax-cache-response
 * @requires RequestStorage
 */
(function($)
{
  /**
   ...
   *
   * @param {Object} obj The object to create a cache id for
   */
  ...
  /**
   * Private method to check if the cache item exists and is still valid.
   * Validity can be configured to use a timeout or invalidated based
   * on a user contributed callback.
   *
   * @param {String} cacheId The stringified cache id
   * @param {Object} options The ajax settings object
   */
   ...
   *
   * @memberOf jQuery
   * @link http://api.jquery.com/jQuery.ajaxPrefilter/
   */
  $.ajaxPrefilter(function(options, originalOptions, jqXhr)
  {
    if(options.cacheResponse === true)
    {
      ...
      {
        throw Exception("No valid storage defined for the Ajax Cache Response plugin");
      }
