...
$t->is(truncate_text($text, 30, '...'), $truncated, 'text_truncate() adds ... to truncated text');
...
$t->is(truncate_text($text, 15, '...', false), $truncated_false, 'text_truncate() does not cut text inside words (its fourth argument)');
...
// utf8 tests!
$t->is(excerpt_text("Tohle je nádherné ráno", "večer"), '', 'text_excerpt() does nothing if the search string is not in input');
$t->is(excerpt_text("Tohle je nádherné ráno", "tohle", 5, '...', true), "Tohle je ...", 'text_excerpt() takes a fifth argument allowing excerpt on whitespace');
$t->is(excerpt_text("Tohle je nádherné ráno", "nádherné", 3, '...', true), "... nádherné ...", 'text_excerpt() takes a fifth argument allowing excerpt on whitespace');
$t->is(excerpt_text("Tohle je nádherné ráno", "ráno", 5), "...erné ráno", 'text_excerpt() creates an excerpt of a text');
