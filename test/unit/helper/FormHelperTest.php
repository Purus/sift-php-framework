...
// form_tag()
$t->diag('form_tag()');
$t->is(form_tag(), '<form method="post" accept-charset="UTF-8" action="module/action">', 'form_tag() creates a form tag');

// options
$t->is(form_tag('', array('class' => 'foo')), '<form class="foo" method="post" accept-charset="UTF-8" action="module/action">', 'form_tag() takes an array of attribute options');
$t->is(form_tag('', array('method' => 'get')), '<form method="get" accept-charset="UTF-8" action="module/action">', 'form_tag() takes a "method" as an option');
$t->is(form_tag('', array('multipart' => true)), '<form method="post" enctype="multipart/form-data" accept-charset="UTF-8" action="module/action">', 'form_tag() takes a "multipart" boolean option');
...
// submit_tag()
$t->diag('submit_tag()');
$t->is(submit_tag(), '<button type="submit" name="commit" value="Save changes"><span>Save changes</span></button>', 'submit_tag() default value is "Save changes"');
$t->is(submit_tag("save"), '<button type="submit" name="commit" value="save"><span>save</span></button>', 'submit_tag() takes a value as its first argument');

// options
$t->is(submit_tag('save', array('class' => 'foo')), '<button type="submit" name="commit" value="save" class="foo"><span>save</span></button>', 'submit_tag() takes an array of attribute options as its second argument');
$t->is(submit_tag('save', array('name' => 'foo')), '<button type="submit" name="foo" value="save"><span>save</span></button>', 'submit_tag() can override the "name" attribute');

// reset_tag()
$t->diag('reset_tag()');
$t->is(reset_tag(), '<button type="reset" name="reset" value="Reset"><span>Reset</span></button>', 'reset_tag() default value is "Reset"');
$t->is(reset_tag("save"), '<button type="reset" name="reset" value="save"><span>save</span></button>', 'reset_tag() takes a value as its first argument');

// options
$t->is(reset_tag('save', array('class' => 'foo')), '<button type="reset" name="reset" value="save" class="foo"><span>save</span></button>', 'reset_tag() takes an array of attribute options as its second argument');
$t->is(reset_tag('save', array('name' => 'foo')), '<button type="reset" name="foo" value="save"><span>save</span></button>', 'reset_tag() can override the "name" attribute');
