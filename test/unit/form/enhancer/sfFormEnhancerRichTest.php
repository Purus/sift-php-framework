...
// load for common cultures
$dateFormatter = new sfI18nDateFormatter('cs_CZ');
$pattern = $dateFormatter->getInputPattern('g');

foreach($dateFormats as $format => $expected)
{
  $t->is(sfFormEnhancerTest::convertDateFormat($format, 'g'), $expected, sprintf('format "%s" is converted to jquery ui date format successfully', $format));
}
//
//sfConfig::add(array(
//  'sf_culture' => 'cs_CZ'
