...
class MyFormatter extends sfWidgetFormSchemaFormatter
{
  ...
  public function unnestErrors($errors, $prefix = '', $widgetAttributes = array())
  {
    return parent::unnestErrors($errors, $prefix, $widgetAttributes);
  }
...
$t->is($f->generateLabel('first_name'), '<label for="first_name" id="first_name_label">Your First Name</label>', '->generateLabelName() returns a label tag');
$t->is($f->generateLabel('first_name', array('class' => 'foo')), '<label class="foo" for="first_name" id="first_name_label">Your First Name</label>', '->generateLabelName() returns a label tag with optional HTML attributes');
$t->is($f->generateLabel('first_name', array('for' => 'myid')), '<label for="myid" id="myid_label">Your First Name</label>', '->generateLabelName() returns a label tag with specified for-id');

$w->setLabel('last_name', 'Your Last Name');
$t->is($f->generateLabel('last_name'), '<label for="last_name" id="last_name_label">Your Last Name</label>', '->generateLabelName() returns a label tag');
MyFormatter::setTranslationCallable('my__');
$t->is($f->generateLabel('last_name'), '<label for="last_name" id="last_name_label">[Your Last Name]</label>', '->generateLabelName() returns a i18ned label tag');
