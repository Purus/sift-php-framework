...
$output = <<<EOF
<tr>
  <th><label for="first_name" class="required">First name <span class="required"><span>*</span></span></label></th>
  <td><input type="text" name="first_name" value="Fabien" id="first_name" /></td>
</tr>
<tr>
  <th><label for="last_name" class="required">Last name <span class="required"><span>*</span></span></label></th>
...
{
  $t->pass('calling of undefined method throws an exception');
}
...
try 
{
  ...
  $t->is($result, 'foobar2', 'method listener returns correct value');
}
catch(sfException $e)
{
...
$t->isa_ok($f->addGroup('test_group', 'My label', array('a')), 'FormTest', 'addGroup() adds group to the form');
...
{
  $t->pass('getGroup() throws an exception when this group does not exist');
}

$t->diag('->getErrors()');

$t->isa_ok($f->getErrors(), 'array', 'getErrors() returns an array of errors');

$t->diag('->filter');

function myFormListener($event, $values)
{
  $values['a'] = 'bbbbbb';
  return $values;
}
...
$t->is($f->getValues(), array('a' => 'bbbbbb', 'b' => 'bbb', 'c' => 'ccccc'), 'event system works ok for value filtering');

$t->is($f->getValue('a'), 'bbbbbb', 'event system works ok for value filtering');
