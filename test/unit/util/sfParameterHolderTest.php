...
$ph = new sfParameterHolder();
$ph->set('myfoo', 'bar', 'sift/mynamespace');
$t->is('bar', $ph->get('myfoo', null, 'sift/mynamespace'), '->get() takes an optional namespace as its third argument');
...
$t->is($ph->getNames('sift/mynamespace'), array('myfoo'), '->getNames() takes a namepace as its first argument');
...
$t->is($ph->getNamespaces(), array($ph->getDefaultNamespace(), 'sift/mynamespace'), '->getNamespaces() returns all non empty namespaces');
...
$t->is($ph->has('myfoo', 'sift/mynamespace'), true, '->has() returns true if the key exists in the namespace given as its second argument');
...
$t->is($ph->hasNamespace('sift/mynamespace'), true, '->hasNamespace() returns true if the namespace exists');
$t->is($ph->hasNamespace('sift/nonexistant'), false, '->hasNamespace() returns false if the namespace does not exist');
...
$t->is($ph->has('myfoo', 'sift/mynamespace'), true, '->remove() removes the key from parameters for a given namespace');

$ph->remove('myfoo', 'sift/mynamespace');
$t->is($ph->has('myfoo', 'sift/mynamespace'), false, '->remove() takes a namespace as its second argument');
...
$t->is($ph->has('myfoo', 'sift/mynamespace'), true, '->removeNamespace() does not remove keys in other namepaces');
...
$ph->removeNamespace('sift/mynamespace');
$t->is($ph->has('myfoo', 'sift/mynamespace'), false, '->removeNamespace() takes a namespace as its first parameter');
...
$ph->set('myfoo', 'bar', 'sift/mynamespace');
$t->is($ph->get('myfoo', null, 'sift/mynamespace'), 'bar', '->set() takes a namespace as its third parameter');
...
$myfoo = 'bar';
$ph->setByRef('myfoo', $myfoo, 'sift/mynamespace');
$t->is($ph->get('myfoo', null, 'sift/mynamespace'), $myfoo, '->setByRef() takes a namespace as its third parameter');
...
$ph = new sfParameterHolder();
$ph->add($parameters);
$ph->add($myparameters, 'sift/mynamespace');

$t->is($ph->getAll(), $parameters, '->add() adds an array of parameters');
$t->is($ph->getAll('sift/mynamespace'), $myparameters, '->add() takes a namespace as its second argument');
...
$ph->addByRef($myparameters, 'sift/mynamespace');

$t->is($parameters, $ph->getAll(), '->add() adds an array of parameters');
$t->is($myparameters, $ph->getAll('sift/mynamespace'), '->add() takes a namespace as its second argument');
