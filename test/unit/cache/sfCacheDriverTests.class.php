...
class sfCacheDriverTests
{
  ...
  // ->set() ->get() ->has()
    $t->diag('->set() ->get() ->has()');
    $data = 'some random data to store in the cache system... (\'"!#/Ă©$ÂŁ)';
    ...
    $cache->clean(sfCache::MODE_OLD);
    $t->is($cache->has('foo'), false, '->clean() cleans old cache key if given the sfCache::MODE_OLD argument');
    $t->is($cache->has('bar'), true, '->clean() cleans old cache key if given the sfCache::MODE_OLD argument');

    $cache->set('foo', $data, -10);
    $cache->set('bar', $data, 86400);

    $cache->clean(sfCache::MODE_ALL);
    $t->is($cache->has('foo'), false, '->clean() cleans all cache key if given the sfCache::MODE_ALL argument');
    $t->is($cache->has('bar'), false, '->clean() cleans all cache key if given the sfCache::MODE_ALL argument');
