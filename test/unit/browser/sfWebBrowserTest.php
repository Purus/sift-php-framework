...
foreach($adapter_list as $adapter)
{
  ...
  $t->isa_ok($b->getResponseXml(), 'sfXmlElement', 'getResponseXml() returns the response as a sfXmlElement');
  ...
  {
    $t->todo('Cookies can be added to the request (curl driver only)');
  }
  ...
  {
    $t->todo('The request can upload a file (curl driver only)');
  }
