...
class sfWebRequest extends sfRequest
{
    ...
    {
        if ($this->ip_forwarded_for != false && $this->ip_forwarded_for != null) {
    ...
    public function getFiles($key = null)
    {
        ...
        return null === $key
            ? $this->fixedFileArray : (isset($this->fixedFileArray[$key]) ? $this->fixedFileArray[$key] : array());
    }
    ...
    protected function loadParameters()
    {
        ...
        if (sfConfig::get('sf_logging_enabled')) {
            sfLogger::getInstance()->info(
                sprintf(
                    '{sfRequest} Request parameters %s',
    ...
    {
        if ($this->hasFile($name) && $this->getFileValue($name, 'error') == UPLOAD_ERR_OK
            && $this->getFileValue($name, 'size') > 0
        ) {
    ...
    public function getHost()
    {
        ...
        return isset($pathArray['HTTP_X_FORWARDED_HOST']) ? $pathArray['HTTP_X_FORWARDED_HOST']
            : (isset($pathArray['HTTP_HOST']) ? $pathArray['HTTP_HOST'] : '');
    }
    ...
    public function isSecure()
    {
        ...
        return (
            (isset($pathArray['HTTPS'])
                && (strtolower($pathArray['HTTPS']) == 'on'
                    || strtolower($pathArray['HTTPS']) == 1))
    ...
    public function getRelativeUrlRoot()
    {
        if ($this->relativeUrlRoot === null) {
            $this->relativeUrlRoot = sfConfig::get(
