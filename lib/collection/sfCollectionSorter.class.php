    ...
    public static function factory($strategy, $strategyArguments = array())
    {
        ...
        if (!class_exists($strategyClass)) {
            throw new InvalidArgumentException(sprintf(
                'Sorting strategy "%s", class "%s" does not exist.',
                $strategy,
                $strategyClass
            ));
        }
        ...
        if (!$reflection->isInstantiable()
            ...
            throw new InvalidArgumentException(sprintf(
                'Sorting stragery "%s" is not valid. It is not instantiable or does not implement sfICollectionSorterStrategy interface.',
                $strategy
            ));
        }
