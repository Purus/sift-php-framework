...
abstract class sfApplication extends sfProject
{
    ...
    protected function initConfiguration()
    {
        ...
        if (!sfConfig::get('sf_debug')) {
            set_error_handler(
                array('sfPhpErrorException', 'handleErrorCallback'),
                sfConfig::get('sf_error_reporting', E_ALL & ~E_NOTICE & ~E_WARNING)
            );
        } else {
        ...
        $this->configCache->import(
            sprintf($sf_app_config_dir_name . '/%s/modules.yml', sfConfig::get('sf_app')),
            true,
            true
        );
    ...
    protected function initOptions()
    {
        ...
        $this->addOptions(
            array(
                ...
                'sf_cache_dir'          =>
                    $sf_cache_dir = ($this->getOption('sf_base_cache_dir') . DS . $this->getEnvironment()
        ...
        switch ($pathInfo) {
            ...
            default:
                throw new sfConfigurationException(sprintf(
                    'Invalid configuration value "%s" for "sf_path_info_array". Valid values are "SERVER" or "ENV"',
                    $pathInfo
                ));
        }
    ...
    {
        if (sfToolkit::hasLockFile(
                $this->getOption('sf_data_dir') . DIRECTORY_SEPARATOR . $this->getOption('sf_app') . '_'
                . $this->getEnvironment() . '-cli.lck',
                5
            )
