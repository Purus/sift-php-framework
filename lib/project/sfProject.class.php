...
abstract class sfProject extends sfConfigurable
{
    ...
    public function setup()
    {
        if (!$this->getOption('sf_apps_dir')) {
            $this->setOption(
                'sf_apps_dir',
                $this->getOption('sf_root_dir') . DS . $this->getOption('sf_apps_dir_name')
            );
        }
        ...
        if (!$this->getOption('sf_config_dir')) {
            $this->setOption(
                'sf_config_dir',
                $this->getOption('sf_root_dir') . DS . $this->getOption('sf_config_dir_name')
            );
        }
        ...
        if (!$this->getOption('sf_model_lib_dir')) {
            $this->setOption(
                'sf_model_lib_dir',
                $this->getOption('sf_lib_dir') . DS . $this->getOption('sf_model_dir_name')
            );
        }
    ...
    public function initializeAutoload($reload = false)
    {
        ...
        if (!is_readable($cacheFile)) {
            ...
            foreach ($this->getPlugins() as $plugin) {
                // load from plugins
                if (is_readable(
                    $file = $plugin->getRootDir() . '/' . $this->getOption('sf_config_dir_name') . '/autoload.yml'
                )
                ) {
                    $files[] = $file;
                }
        ...
        if (!isset($this->applications[$application]) || $forceReload) {
            ...
            if (!is_dir($this->getOption('sf_apps_dir') . '/' . $application)) {
                throw new sfConfigurationException(sprintf(
                    'Application "%s" does not exist in the app directory "%s"',
                    $application,
                    $this->getOption('sf_apps_dir') . '/' . $application
                ));
            }
    ...
    protected function getPluginsConfigurationFiles()
    {
        ...
        if (is_readable(
            $file = $this->getOption('sf_root_dir') . DS .
        ...
        if ($this->getOption('sf_app_config_dir')
            && is_readable(
                $file = $this->getOption('sf_app_config_dir') . '/plugins.yml'
            )
        ) {
            $files[] = $file;
        }
