...
class sfCollator
{
    ...
    public function __construct($culture)
    {
        if (class_exists('Collator', false)) {
            ...
            if (intl_is_failure($this->collator->getErrorCode())
                || strpos($culture, $this->collator->getLocale(Locale::VALID_LOCALE)) === false
            ) {
                throw new RuntimeException(sprintf(
                    'Error creating collator for culture "%s". %s',
                    $culture,
    ...
    public function compare($string1, $string2)
    {
        ...
        if (strpos(PHP_OS, 'WIN') !== false) {
            ...
            setlocale(
                LC_COLLATE,
                sprintf('%s.1250', $this->getWindowsLocale($this->getCulture())),
                $this->getCulture(),
