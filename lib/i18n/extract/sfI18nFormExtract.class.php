...
class sfI18nFormExtract extends sfI18nExtract
{
    ...
    public function configure()
    {
        ...
        if (!is_dir($this->cataloguePath)) {
            throw new sfConfigurationException(sprintf(
                'The form catalogue specifies the path "%s" which does not exist.',
                $this->cataloguePath
            ));
        }
        ...
        if (!$reflection->isSubclassOf('sfII18nExtractableForm')) {
            ...
            if ($cannotCreate) {
                throw new sfException(sprintf(
                    'The form "%s" cannot be extracted. Constructor arguments disallow standard way of extraction. Please implement sfII18nExtractable interface to the form.',
                    $class
                ));
            }
        }
    ...
    private function merge($field)
    {
        ...
        if (method_exists($field, 'getActiveMessages') && method_exists($field, 'getValidators')) {
            $this->strings[$this->catalogue] = array_merge(
                $this->strings[$this->catalogue],
                $field->getActiveMessages()
            );
