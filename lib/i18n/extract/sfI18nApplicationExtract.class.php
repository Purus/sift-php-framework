...
class sfI18nApplicationExtract extends sfI18nExtract
{
    ...
    {
        $extracted = $this->extractFromPhpFiles(
            array(
    ...
    protected function extractMenuYamlFiles()
    {
        ...
        // menu yaml files
        $menuFiles = sfFinder::type('file')->name('menu.yml')->in(
            $this->getOption('app_dir') . '/' .
    ...
    protected function extractDashboardWidgetsYamlFiles()
    {
        ...
        // menu yaml files
        $menuFiles = sfFinder::type('file')->name('dashboard_widgets.yml')->in(
            $this->getOption('app_dir') . '/' .
            $this->getOption('config_dir_name')
        );
        foreach ($menuFiles as $file) {
    ...
    protected function extractUserProfileYamlFiles()
    {
        ...
        $userProfileFiles = sfFinder::type('file')->name('user_profile.yml')->in(
            $this->getOption('app_dir') . '/' .
            $this->getOption('config_dir_name')
        );
            ...
            foreach ($classes as $class) {
                try {
                    ...
                    throw new sfException(
                        sprintf(
                            'Error extracting form "%s". Original message was: %s',
                            $class,
                            $e->getMessage()
                        ), $e->getCode());
                }
        ...
        foreach ($files as $file) {
            ...
            foreach ($classes as $class) {
                try {
                    ...
                    throw new sfException(
                        sprintf(
                            'Error extracting model "%s". Original message was: %s',
                            $class,
                            $e->getMessage()
                        ), $e->getCode());
                }
    ...
    protected function parseAllSeenMessages()
    {
        foreach ($this->allSeenMessages as $catalogue => $messages) {
            ...
            $this->oldMessages[$catalogue] = array_diff(
                $this->currentMessages[$catalogue],
                $this->allSeenMessages[$catalogue]
            );
    ...
    protected function sortExtracted($extracted, $context = 'application', $module = null)
    {
        foreach ($extracted as $domain => $messages) {
            ...
            if ($context == 'plugin' && $this->getOption('plugin')) {
                $isPluginCatalogue
                    = strpos(str_replace(DIRECTORY_SEPARATOR, '/', $domain), $this->getOption('plugin')) !== false;
            }

            $domain = $this->replaceConstants($domain);

            // we have global application catalogue
            if ($domain == self::UNKNOWN_DOMAIN || (strpos($domain, '/') === false && $domain == $this->catalogueName)
                || $isPluginCatalogue
            ) {
                switch ($context) {
                    case 'application':
