...
/**
 * sfI18nNumberFormatter class.
 *
 * sfI18nNumberFormatter decimal numbers in any locale. The decimal
 * number is formatted according to a particular pattern. These
 * patterns can arise from the sfI18nNumberFormat object which is
 * culturally sensitive. The sfI18nNumberFormatter class can be instantiated in
 * many ways. E.g.
 *
 * <code>
 *  // create a invariant number formatter.
 *  $formatter = new sfI18nNumberFormatter();
 *
 ...
 * <code>
 *  $ja = new sfI18nNumberFormatter('ja_JP');
 *
...
class sfI18nNumberFormatter
{
     ...
     * a sfCulture or sfI18nNumberFormat instance will instantiated a instance
    ...
    public function __construct($formatInfo = null)
    {
        if (is_null($formatInfo)) {
            ...
            if ($formatInfo instanceof sfI18nCulture) {
                $this->formatInfo = $formatInfo->sfNumberFormat;
            } else {
                if ($formatInfo instanceof sfI18nNumberFormat) {
    ...
    public function format($number, $pattern = 'd', $currency = 'USD', $charset = 'UTF-8')
    {
        ...
        return sfI18n::i18n2Encoding($result, $charset);
    }
     ...
     * are retrieved from the sfI18nNumberFormat instance.
     *
    ...
    protected function setPattern($pattern)
    {
        switch ($pattern) {
            ...
            case 'd':
            case 'D':
                $this->formatInfo->setPattern(sfI18nNumberFormat::DECIMAL);
                break;
            case 'e':
            case 'E':
                $this->formatInfo->setPattern(sfI18nNumberFormat::SCIENTIFIC);
                break;
            case 'p':
            case 'P':
                $this->formatInfo->setPattern(sfI18nNumberFormat::PERCENTAGE);
                break;
            default:
