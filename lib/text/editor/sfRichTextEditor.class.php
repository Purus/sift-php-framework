...
abstract class sfRichTextEditor extends sfConfigurable implements sfIRichTextEditor
{
        ...
        if ($driverObj) {
            if (!$driverObj instanceof sfIRichTextEditor) {
                throw new LogicException(sprintf(
                    'Driver "%s" does not implement sfIRichTextEditor interface.',
                    $driver
                ));
            }
    ...
    public function __construct($options = array())
    {
        ...
        if (sfCore::isBootstrapped()) {
            ...
            sfCore::dispatchEvent(
                'rich_text_editor.load_options',
                array(
                    'editor' => $this
                )
            );
    ...
    public function loadOptions()
    {
        ...
        array_walk_recursive(
            $config,
            create_function('&$config', '$config = sfToolkit::replaceConstantsWithModifiers($config);')
        );
