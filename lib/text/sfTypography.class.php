...
class sfTypography
{
    ...
    public function __construct($language = null, $options = array())
    {
        ...
        foreach ($lang as $_language) {
            $languageFile = sprintf(
                '%s/i18n/typo/%s.dat',
                sfConfig::get('sf_sift_data_dir'),
                $_language
            );
    ...
    public static function getInstance($language = null, $options = array())
    {
        ...
        if ((count(self::$store) <= 0) || (!array_key_exists($language, self::$store))
            || (!is_object(
                self::$store[$language]
            ))
     ...
     */
    public static function correctStatic(
        $string,
        $options = null,
        $apllyFilters = null
    ) {
    ...
    private function doHyphenateWord($word)
    {
        ...
        for ($i = 1; $i < $wl; $i++) {
            ...
            if ((0 !== $positions[$i]) && (1 === ($positions[$i] % 2))
    ...
    private function getHyphenationPositions($word)
    {
        ...
        for ($s = 0; $s < $wl - 1; $s++) {
            ...
            for (
                $l = $this->options['hyphenate_shortest_pattern'];
    ...
    private function isNotToBeHyphenated($word)
    {
        if ((null === $this->options['no_hyphenate_string'])
            || (0 !== strpos(
                    $word,
                    $this->options['no_hyphenate_string']
                ))
        ) {
            return false;
        }
    ...
    {
        if ((null === $this->options['custom_hyphen']) || (false === strpos($word, $this->options['custom_hyphen']))) {
            return false;
        }
