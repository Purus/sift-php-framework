...
class sfPlainTextTable
{
     ...
     * @protected array
     */
    protected $_cell_lengths = array();
    /**
     * Heights of the rows.
     *
     * @protected array
     */
    protected $_row_heights = array();
    /**
     ...
     *                         self::ALIGN_LEFT,
     *                         self::BORDER_ASCIIALIGN_CENTER or
     *                         self::BORDER_ASCIIALIGN_RIGHT.
     * @param string  $border  The character used for table borders or
     *                         self::BORDER_ASCIIBORDER_ASCII.
        ...
        for ($i = 0; $i < $this->_max_rows; $i++) {
            for ($j = 0; $j < $this->_max_cols; $j++) {
                if (!isset($this->_data[$i][$j])
                    && (!isset($this->_data[$i]) || $this->_data[$i] !== self::HORIZONTAL_RULE)
                ) {
                    $this->_data[$i][$j] = '';
                }
            }
    ...
    protected function _buildTable()
    {
        ...
        $rule = $this->_border == self::BORDER_ASCII ? '|' : $this->_border;
    ...
    protected function _strlen($str)
    {
        ...
        if ($utf8
            && ($this->_charset == strtolower('utf-8') || $this->_charset == strtolower('utf8'))
        ) {
