...
class sfRouting
{
    ...
    public function getCurrentInternalUri($with_route_name = false)
    {
        if ($this->current_route_name) {
            list($url, $regexp, $names, $names_hash, $defaults, $requirements, $suffix)
                = $this->routes[$this->current_route_name];
            ...
            if ($with_route_name) {
                ...
                $internal_uri
                    = $request->getParameter('module', isset($defaults['module']) ? $defaults['module'] : '') . '/'
                    . $request->getParameter('action', isset($defaults['action']) ? $defaults['action'] : '');
            }
            ...
            foreach ($names as $name) {
                ...
                $params[]
                    = $name . '=' . $request->getParameter($name, isset($defaults[$name]) ? $defaults[$name] : '');
            }
