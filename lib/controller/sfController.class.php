...
abstract class sfController implements sfIController
{
    ...
    protected function controllerExists($moduleName, $controllerName, $extension, $throwExceptions)
    {
        ...
        foreach ($dirs as $dir => $checkEnabled) {
            ...
            if ($checkEnabled && !in_array($moduleName, sfConfig::get('sf_enabled_modules', array()))
                && is_readable(
                    $dir
                )
            ) {
            ...
            if (is_readable($file)) {
                // action class exists
                require_once($file);
                $this->controllerClasses[$moduleName . '_' . $controllerName . '_' . $classSuffix]
                    = $controllerName . $classSuffix;
            ...
            if (is_readable($module_file)) {
                ...
                if (!class_exists($moduleName . $classSuffix . 's', false)) {
                    if ($throwExceptions) {
                        throw new sfControllerException(sprintf(
        ...
        if ($throwExceptions && sfConfig::get('sf_debug')) {
            ...
            throw new sfControllerException(sprintf(
                '{sfController} controller "%s/%s" does not exist in: %s',
                $moduleName,
                $controllerName,
                implode(', ', $dirs)
            ));
    ...
    public function forward($moduleName, $actionName)
    {
        ...
        if ($this->getActionStack()->getSize() >= $this->maxForwards) {
            ...
            throw new sfForwardException(sprintf(
                'Too many forwards have been detected for this request (> %d)',
                $this->maxForwards
            ));
        }
        ...
        if (!sfConfig::get('sf_available')
            ...
            if (!$this->actionExists($moduleName, $actionName)) {
                ...
                throw new sfConfigurationException(sprintf(
                    'Invalid configuration settings: [sf_unavailable_module] "%s", [sf_unavailable_action] "%s"',
                    $moduleName,
                    $actionName
                ));
            }
        ...
        sfConfigCache::getInstance()->import(
            sfConfig::get('sf_app_module_dir_name') . '/' . $moduleName . '/' . sfConfig::get(
        ...
        if (!$this->actionExists($moduleName, $actionName)) {
            ...
            if (sfConfig::get('sf_logging_enabled')) {
                sfLogger::getInstance()->info(
                    sprintf('{sfController} The action "%s/%s" does not exist', $moduleName, $actionName)
                );
            }
            ...
            if (!$this->actionExists($moduleName, $actionName)) {
                // cannot find unavailable module/action
                throw new sfConfigurationException(sprintf(
                    'Invalid configuration settings: [sf_error_404_module] "%s", [sf_error_404_action] "%s"',
                    $moduleName,
                    $actionName
                ));
            }
        ...
        if ($this->getActionStack()->getSize() == 1 && sfConfig::get('mod_' . strtolower($moduleName) . '_is_internal')
            ...
            throw new sfConfigurationException(sprintf(
                'Action "%s" from module "%s" cannot be called directly',
                $actionName,
                $moduleName
            ));
        }
        ...
        if (sfConfig::get('mod_' . strtolower($moduleName) . '_enabled')) {
            ...
            $moduleConfig = sfConfig::get('sf_app_module_dir') . '/' . $moduleName . '/' . sfConfig::get(
                    'sf_app_module_config_dir_name'
                ) . '/config.php';
    ...
    public function getView($moduleName, $actionName, $viewName)
    {
        ...
        $file
            =
            sfConfig::get('sf_app_module_dir') . '/' . $moduleName . '/' . sfConfig::get('sf_app_module_view_dir_name')
        ...
        if (is_readable($file)) {
            ...
            $class = sprintf(
                '%sView', // view class (as configured in module.yml or defined in action)
                $this->getContext()->getRequest()->getAttribute(
                    $moduleName . '_' . $actionName . '_view_name',
    ...
    public function getPresentationFor($module, $action, $viewName = null)
    {
        if (sfConfig::get('sf_logging_enabled')) {
            sfLogger::getInstance()->info(
                '{sfController} Get presentation for action "' . $module . '/' . $action . '" (view class: "'
                . $viewName . '")'
            );
        }
        ...
        try {
            ...
            if ($viewName) {
                ...
                $this->getContext()->getRequest()->getAttributeHolder()->remove(
                    $module . '_' . $action . '_view_name',
                    'sift/action/view'
                );
            }
        ...
        while ($nb-- > 0) {
            ...
            if ($actionEntry->getModuleName() == sfConfig::get('sf_login_module')
                && $actionEntry->getActionName() == sfConfig::get('sf_login_action')
            ) {
                throw new sfException('Your action is secured, but the user is not authenticated.');
            } elseif ($actionEntry->getModuleName() == sfConfig::get('sf_secure_module')
                && $actionEntry->getActionName() == sfConfig::get('sf_secure_action')
            ) {
    ...
    public function __call($method, $arguments)
    {
        $event = sfCore::getEventDispatcher()->notifyUntil(
            new sfEvent('controller.method_not_found', array(
                'method'     => $method,
                'arguments'  => $arguments,
                'controller' => $this
            ))
