    ...
    public function execute($configFiles)
    {
        ...
        foreach ($config as $file) {
            ...
            $contents = preg_replace_callback(
                array(
                    '#(require|include)(_once)?\((sfConfigCache::getInstance\(\)|\$configCache)->checkConfig\([^_]+sf_app_config_dir_name[^\.]*\.\'/([^\']+)\'\)\);#m',
                    '#()()(sfConfigCache::getInstance\(\)|\$configCache)->import\(.sf_app_config_dir_name\.\'/([^\']+)\'(, false)?\);#m'
                ),
                array(
                    $this,
                    'insertConfigFileCallback'
                ),
                $contents
            );

            // strip php tags
            $contents = sfToolkit::pregtr(
                $contents,
                array(
                    '/^\s*<\?(php)?/m' => '',
                    '/^\s*\?>/m'       => ''
        ...
        // save current sift release
        file_put_contents(sfConfig::get('sf_config_cache_dir') . '/VERSION', sfCore::getVersion());
