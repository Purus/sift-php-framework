    ...
    public function execute($configFiles)
    {
            ...
            foreach ($config['set'] as $key => $value) {
                ...
                if (!array_key_exists($key, $configs)) {
                    ...
                    throw new sfParseException(sprintf(
                        'Configuration file "%s" specifies key "%s" which is not a php.ini directive.',
                        $configFiles[0],
                        $key
                    ));
                }
                ...
                if ($configs[$key]['access'] != 7 && $configs[$key]['access'] != 63) {
                    throw new sfParseException(sprintf(
                        'Configuration file "%s" specifies key "%s" which cannot be overrided.',
                        $configFiles[0],
                        $key
                    ));
                }
            ...
            foreach ($config['check'] as $key => $value) {
                ...
                if (ini_get($key) != $value) {
                    throw new sfInitializationException(sprintf(
                        'Configuration file "%s" specifies that php.ini "%s" key must be set to "%s". The current value is "%s" (%s).',
                        $configFiles[0],
                        $key,
                        $this->varExport($value),
                        $this->convertToString($this->varExport(ini_get($key))),
                        $this->getIniPath()
                    ));
                }
            }
        ...
        if (isset($config['warn'])) {
            foreach ($config['warn'] as $key => $value) {
                ...
                $warning = sprintf(
                    '{sfPhpConfigHandler} php.ini "%s" key is better set to "%s" (current value is "%s" - %s).',
                    $key,
                    $this->varExport($value),
                    $this->varExport(ini_get($key)),
                    $this->getIniPath()
                );
                $data[] = sprintf(
        ...
        if (isset($config['extensions'])) {
            foreach ($config['extensions'] as $extension_name) {
                if (!extension_loaded($extension_name)) {
                    throw new sfInitializationException(sprintf(
                        'Configuration file "%s" specifies that the PHP extension "%s" should be loaded. (%s).',
                        $configFiles[0],
                        $extension_name,
                        $this->getIniPath()
                    ));
