...
class sfViewConfigHandler extends sfYamlConfigHandler
{
    ...
    private function addBodyAttributes($viewName = '')
    {
        ...
        if ((!is_null($body_classes) && !is_array($body_classes)) && sfConfig::get('sf_environment') == 'dev') {
            throw new sfConfigurationException(sprintf(
                'Body classes has been misconfigured. "body_class" setting should be an array. "%s" given for view: "%s"',
                gettype($body_classes),
                $viewName
            ));
        }
    ...
    protected function mergeConfig($myConfig)
    {
        ...
        $myConfig['all']['stylesheets'] = array_merge(
            isset($myConfig['default']['stylesheets']) && is_array($myConfig['default']['stylesheets'])
                ? $myConfig['default']['stylesheets'] : array(),
            isset($myConfig['all']['stylesheets']) && is_array($myConfig['all']['stylesheets'])
                ? $myConfig['all']['stylesheets'] : array()
        );
        unset($myConfig['default']['stylesheets']);

        $myConfig['all']['javascripts'] = array_merge(
            isset($myConfig['default']['javascripts']) && is_array($myConfig['default']['javascripts'])
                ? $myConfig['default']['javascripts'] : array(),
            isset($myConfig['all']['javascripts']) && is_array($myConfig['all']['javascripts'])
                ? $myConfig['all']['javascripts'] : array()
        );
    ...
    protected function addLayout($viewName = '')
    {
        ...
        $hasLocalLayout = isset($this->yamlConfig[$viewName]['layout'])
            || (isset($this->yamlConfig[$viewName])
                && array_key_exists('has_layout', $this->yamlConfig[$viewName]));

        // the layout value
        $layout = $this->getConfigValue('has_layout', $viewName) ? $this->replaceConstants(
            $this->getConfigValue('layout', $viewName)
        ) : false;
...
EOF;
    ...
    protected function addHtmlHead($viewName = '')
    {
        ...
        foreach ($this->mergeConfigValue('http_metas', $viewName) as $httpequiv => $content) {
            $data[] = sprintf(
                "  \$response->addHttpMeta('%s', '%s', false);",
                $httpequiv,
                str_replace('\'', '\\\'', $content)
            );
        }
        ...
        foreach ($this->mergeConfigValue('metas', $viewName) as $name => $content) {
            if ($name == 'title') {
                $data[] = sprintf(
                    "  \$response->setTitle('%s', true, false, true);",
                    str_replace('\'', '\\\'', preg_replace('/&amp;(?=\w+;)/', '&', $content))
                );
            } else {
                $data[] = sprintf(
                    "  \$response->addMeta('%s', '%s', false, false);",
                    $name,
                    str_replace(
                        '\'',
                        '\\\'',
        ...
        if (!empty($title)) {
            $data[] = sprintf(
                "  \$response->setTitle('%s', true, false, true);",
                str_replace('\'', '\\\'', preg_replace('/&amp;(?=\w+;)/', '&', $title))
            );
        }
    ...
    protected function addHtmlAsset($viewName = '')
    {
        ...
        foreach ($packages as $package) {
            $packageStylesheets = array_merge(
                $packageStylesheets,
                sfAssetPackage::getStylesheets($package, true, false)
            );
            $packageJavascripts = array_merge(
                $packageJavascripts,
                sfAssetPackage::getJavascripts($package, true, false)
            );
        }
