...
class sfConfigCache
{
    ...
    public function checkConfig($configPath, $optional = false)
    {
        ...
        if (!is_readable($cache) || $mtime > filemtime($cache)) {
            ...
            // clear process cache
            if ('config/config_handlers.yml' != $configPath && sfConfig::get('sf_use_process_cache')
        ...
        if ($sf_app_module_dir && is_readable($sf_app_module_dir)) {
            ...
            while (($directory = readdir($fp)) !== false) {
                if (!in_array($directory, $ignore)) {
                    $configPath
                        =
                        $sf_app_module_dir . '/' . $directory . '/' . $this->getOption('sf_app_module_config_dir_name')
                        . '/config_handlers.yml';
                    ...
                    if (is_readable($configPath)) {
                        ...
                        $configPath
                            = $this->getOption('sf_app_module_dir_name') . '/' . $directory . '/' . $this->getOption(
