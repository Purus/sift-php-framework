...
class sfFilterConfigHandler extends sfYamlConfigHandler
{
    ...
    protected function addFilter($category, $class, $parameters)
    {
        $code = array();
        $code[] = sprintf(
            'list($class, $parameters) = sfConfig::get(\'sf_%s_filter\', array(\'%s\', %s));',
            sfInflector::tableize($category),
            $class,
            $parameters
        );
        ...
        $code[] = '{';
        $code[] = sprintf(
            '  throw new LogicException(sprintf(\'The filter "%%s" does not implement sfIFilter interface.\', get_class($filter), \'%s\'));',
            $class
        );
        $code[] = '}';
    ...
    protected function addSecurityFilter($category, $class, $parameters)
    {
        ...
        $code[] = '  {';
        $code[]
            = '      throw new LogicException(\'Security is enabled, but your sfUser implementation does not implement sfISecurityUser interface.\');';
        $code[] = '  }';
        $code[] = '  $filter->initialize($this->context, $parameters);';
        $code[] = '  $this->register($filter);';
        $code[] = '}';
