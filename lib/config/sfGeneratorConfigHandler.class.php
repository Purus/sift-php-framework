...
class sfGeneratorConfigHandler extends sfYamlConfigHandler
{
    ...
    public function execute($configFiles)
    {
        ...
        preg_match(
            sprintf('#.*/%s/([^/]+)/#', sfConfig::get('sf_app_module_dir_name')),
            str_replace('\\', '/', $configFiles[0]),
            $match
        );
        ...
        $retval = sprintf(
            $retval,
            date('Y/m/d H:i:s'),
            self::getContent($generatorManager, $config['class'], $generatorParam)
        );
    ...
    public static function replaceConstants($value)
    {
        if (is_array($value)) {
            array_walk_recursive(
                $value,
                create_function('&$value', '$value = sfGeneratorConfigHandler::replaceConstantsForExpressions($value);')
            );
        } else {
