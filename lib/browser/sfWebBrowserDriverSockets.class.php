...
/**
 * Fsockopen driver for sfWebBrowser
 *
 * @package    Sift
 * @subpackage browser
...
class sfWebBrowserDriverSockets implements sfIWebBrowserDriver
{
    ...
    {
        $m_headers = array_merge(
            array('Content-Type' => 'application/x-www-form-urlencoded'),
            $browser->getDefaultRequestHeaders(),
            $browser->initializeRequestHeaders($headers)
        );
        ...
        for ($i = 0; $i < count($response_lines); $i++) {
            ...
            if ($start_body == true) {
                ...
                if ($start_body == false && $response_lines[$i] == '') {
                    $start_body = true;
                } // grab headers
                else {
