...
class sfWebBrowser
{
     ...
     *
     * @throws InvalidArgumentException If driver cannot be found or is invalid
     */
    public function __construct($defaultHeaders = array(), $driverClass = null, $driverOptions = array())
    {
        ...
        if (!$driver) {
            throw new InvalidArgumentException(sprintf(
                'Browser driver is missing or class "%s" does not exist.',
                $driverClass
            ));
        }
        ...
        if (!$driver instanceof sfIWebBrowserDriver) {
            throw new InvalidArgumentException(sprintf(
                'Browser driver "%s" should implement sfIWebBrowserDriver interface.',
                $driverClass
            ));
        }
    ...
    public function call($uri, $method = 'GET', $parameters = array(), $headers = array(), $changeStack = true)
    {
        ...
        if (!isset($urlInfo['host'])) {
            ...
            if ($urlInfo['scheme'] != 'http' && $urlInfo['scheme'] != 'https') {
                throw new Exception('{sfWebBrowser} Can handle only http and https requests.');
            }
        }
        ...
        $browser = $this->driver->call($this, $uri, $method, $parameters, $headers);

        // redirect support
        if ((in_array($browser->getResponseCode(), array(301, 307)) && in_array($method, array('GET', 'HEAD')))
    ...
    public function click($name, $arguments = array())
    {
        ...
        if (!$form = $xpath->query(
            sprintf(
                '//input[((@type="submit" or @type="button") and @value="%s") or (@type="image" and @alt="%s")]/ancestor::form',
                $name,
                $name
            )
        )->item(0)
        ...
        foreach ($xpath->query('descendant::input | descendant::textarea | descendant::select', $form) as $element) {
            ...
            if ($nodeName == 'input'
                ...
                if (
                    $nodeName == 'input'
                    && (($element->getAttribute('type') != 'submit' && $element->getAttribute('type') != 'button')
                        || $element->getAttribute('value') == $name)
    ...
    public function getResponseXML()
    {
        if (!$this->responseXml) {
            // for HTML/XML content, create a DOM object for the response content
            if (preg_match('/(x|ht)ml/i', $this->getResponseHeader('Content-Type'))) {
                $this->responseXml = @simplexml_load_string($this->getResponseText(), 'sfXmlElement');
            }
        }

        // Throw an exception if response is not valid XML
        if (!$this->responseXml || get_class($this->responseXml) != 'sfXmlElement') {
