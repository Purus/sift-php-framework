...
class sfWebBrowserDriverFopen implements sfIWebBrowserDriver {
  ...
  public function call($browser, $uri, $method = 'GET', $parameters = array(), $headers = array())
  {
    ...
    $context = stream_context_create(array('http' => array_merge(
                $this->options, array('method' => $method), array('content' => is_array($parameters) ? http_build_query($parameters) : $parameters), array('header' => $request_headers)
            )));
