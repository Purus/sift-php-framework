...
class sfWebResponse extends sfResponse
{
     ...
     * @throws sfException If fails to set the cookie
     */
    public function setCompressedCookie(
        $name,
        $value,
        $expire = null,
        $path = '/',
        ...
        foreach ($this->cookies as $cookie) {
            ...
            if (sfConfig::get('sf_logging_enabled')) {
                sfLogger::getInstance()->info(
                    sprintf('{sfResponse} Sent cookie "%s":"%s"', $cookie['name'], $cookie['value'])
                );
            }
        }
    }
    ...
    {
        foreach (array('first', '', 'last') as $position) {
            $this->getParameterHolder()->remove(
                $file,
                'helper/asset/auto/stylesheet' . ($position ? '/' . $position : '')
            );
        }
    ...
    public function removeJavascript($file)
    {
        foreach (array('first', '', 'last') as $position) {
            $this->getParameterHolder()->remove(
                $file,
                'helper/asset/auto/javascript' . ($position ? '/' . $position : '')
            );
        }
        ...
        if ($max && ($length = mb_strlen($title, sfConfig::get('sf_charset'))) > $max) {
            if (sfConfig::get('sf_logging_enabled')) {
                sfLogger::getInstance()->notice(
                    sprintf(
                        '{SEO} Page title is too long (%s chars, maximum is %s). Please consider shorter title.',
                        $length,
                        $max
                    )
                );
            ...
            if (sfConfig::get('app_title_auto_trim', false)) {
                ...
                if (sfConfig::get('sf_logging_enabled')) {
                    sfLogger::getInstance()->info(
                        sprintf('{SEO} Page title has been auto trimmed to %s chars (maximum is %s).', $length, $max)
                    );
                }
            }
        }
    ...
    public function setTitleMode($policy)
    {
        ...
        if (in_array($policy, array(self::TITLE_MODE_APPEND, self::TITLE_MODE_PREPEND, self::TITLE_MODE_REPLACE))) {
            ...
            throw new InvalidArgumentException(sprintf(
                'Invalid title mode "%s" given. Valid modes are: %s',
                $policy,
    ...
    {
        return serialize(
            array($this->content, $this->statusCode, $this->statusText, $this->parameterHolder, $this->headerOnly)
        );
    }
    ...
    {
        list($this->content, $this->statusCode, $this->statusText, $this->parameterHolder, $this->headerOnly)
