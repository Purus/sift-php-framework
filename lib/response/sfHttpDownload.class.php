...
class sfHttpDownload extends sfConfigurable implements sfIEventDispatcherAware, sfILoggerAware
{
    ...
    protected function prepareResponse()
    {
        ...
        switch (($cacheControl = $this->getCacheControl())) {
            ...
            case self::CACHE_CONTROL_PRIVATE_NO_EXPIRE:
                ...
                if (($etag = $this->getEtag()) !== false
                    && ($noneMatch = $this->getRequest()->getHttpHeader('IF_NONE_MATCH'))
                ) {
                    if (!$etag) {
        ...
        $this->response->setHttpHeader(
            ...
            sprintf(
                ...
                $this->fixFilename($this->getFilename() ? $this->getFilename() : $this->guessFilename())
            )
        );
        ...
        switch ($cacheControl) {
            ...
            case self::CACHE_CONTROL_NO_CACHE:
                ...
                $this->response->addCacheControlHttpHeader(
                    'no-store, no-cache, no-transform, must-revalidate, proxy-revalidate, post-check=0, pre-check=0'
                );
                $this->response->setHttpHeader('Expires', $this->response->getDate(strtotime('-10 years')));
    ...
    protected function postSend()
    {
        ...
        if ($this->getBandwidth() > 0 || $this->response->isHeaderOnly()) {
            $this->log(
                'Post send, connection status {status}',
                sfILogger::DEBUG,
                array('status' => connection_status())
            );
            if (!$this->aborted) {
    ...
    protected function doDownload()
    {
        ...
        // do we use resume?
        if ($this->useResume() && ($range = $this->getRequest()->getHttpHeader('RANGE'))) {
            ...
            $this->response->setHttpHeader(
                'Content-Range',
                sprintf('bytes %s-%s/%s', $startPoint, $endPoint, $this->size)
            );
        ...
        if (($speedLimit = $this->limitSpeed())) {
            $this->log(
                'Limitting speed to {limit}',
                sfILogger::INFO,
                array('limit' => $this->formatBytes($this->getBufferSize()))
            );
        }
    ...
    public function setBufferSize($bytes)
    {
        if (0 >= $bytes) {
            throw new InvalidArgumentException(sprintf(
                'Buffer size must be greater than 0 bytes ("%s" given)',
                $bytes
            ));
        }
    ...
    public function setLastModified($lastModified)
    {
        ...
        if ($lastModified <= 0) {
            throw new InvalidArgumentException(sprintf(
                'Invalid last modified time "%s" given. The value should be greater than zero',
                $last_modified
            ));
        }
    ...
    public function setCacheControl($cacheControl)
    {
        if (!in_array(
            ...
            throw new InvalidArgumentException(sprintf(
                'Invalid cache control "%s" given. Valid controls are: %s.',
                $cacheControl,
                '"' . join(
                    '", "',
                    array(
    ...
    public function setClientLifetime($lifetime)
    {
        if ($lifetime < 0) {
            throw new InvalidArgumentException(sprintf(
                'Invalid client lifetime value "%s" given. Should be greater than zero',
                $lifetime
            ));
        }
