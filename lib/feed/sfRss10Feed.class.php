...
class sfRss10Feed extends sfRssFeed
{
    ...
    public function toXml()
    {
        ...
        $xml[]
            =
            '  <channel rdf:about="' . htmlspecialchars($this->context->getController()->genUrl($this->getLink(), true))
            . '">';
        $xml[] = '    <title>' . htmlspecialchars($this->getTitle()) . '</title>';
        $xml[] = '    <link>' . htmlspecialchars($this->context->getController()->genUrl($this->getLink(), true))
            . '</link>';
    ...
    protected function getFeedItemSequence()
    {
        ...
        foreach ($this->getItems() as $item) {
            $xml[] = '        <rdf:li rdf:resource="' . htmlspecialchars(
                    $this->context->getController()->genUrl($item->getLink(), true)
                ) . '" />';
        }
    ...
    protected function getFeedElements()
    {
        $xml = array();
        foreach ($this->getItems() as $item) {
            $xml[] = '  <item rdf:about="' . htmlspecialchars(
                    $this->context->getController()->genUrl($item->getLink(), true)
                ) . '">';
            $xml[] = '    <title>' . htmlspecialchars($item->getTitle()) . '</title>';
            $xml[] = '    <link>' . htmlspecialchars($this->context->getController()->genUrl($item->getLink(), true))
                . '</link>';
