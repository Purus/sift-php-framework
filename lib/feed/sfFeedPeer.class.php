...
class sfFeedPeer
{
    ...
    public static function createFromWeb($uri, $options = array())
    {
        if (isset($options['adapter'])) {
            $browser = new sfWebBrowser(array(), $options['adapter'], isset($options['adapter_options'])
                ? $options['adapter_options'] : array());
        } else {
                ...
                if (isset($options['methods'][$detail])) {
                    if ($options['methods'][$detail]) {
                        if (is_array($options['methods'][$detail])) {
                            call_user_func(
                                array($item, $itemMethod),
                                call_user_func_array(
                                    array($object, $options['methods'][$detail][0]),
                                    $options['methods'][$detail][1]
                                )
                            );
                        } else {
                            call_user_func(
                                array($item, $itemMethod),
                                call_user_func(array($object, $options['methods'][$detail]))
                            );
                        }
                    } else {
                    ...
                    call_user_func(
                        array($item, $itemMethod),
                        call_user_func(array('sfFeedPeer', 'getItemFeed' . ucfirst($detail)), $object)
                    );
                }
            }
    ...
    private static function getItemFeedLink($item, $routeName = '', $fallback_url = '')
    {
        if ($routeName) {
            ...
            foreach ($paramNames as $paramName) {
                ...
                if (!$found) {
                    if (array_key_exists($paramName, $defaults)) {
                        ...
                        $error
                            = 'Cannot find a "getFeed%s()" or "get%s()" method for object "%s" to generate URL with the "%s" route';
            ...
            return sfContext::getInstance()->getController()->genUrl(
                $routeName . ($params ? '?' . implode('&', $params) : ''),
                true
            );
        }
