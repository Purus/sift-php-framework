...
class sfRssFeed extends sfFeed
{
    ...
    public function toXml()
    {
        ...
        if ($this->getAuthorEmail()) {
            $xml[] = '    <managingEditor>' . $this->getAuthorEmail() . ($this->getAuthorName() ?
                    ' (' . $this->getAuthorName() . ')' : '') . '</managingEditor>';
        }
        ...
        if ($this->getImage()) {
            ...
            $xml[] = '      <link>' . $this->context->getController()->genUrl($this->getImage()->getLink(), true)
                . '</link>';
        ...
        foreach ($this->getItems() as $item) {
            ...
            if (strpos($this->version, '2.') !== false) {
                ...
                if ($enclosure = $item->getEnclosure()) {
                    $enclosure_attributes = sprintf(
                        'url="%s" length="%s" type="%s"',
                        $enclosure->getUrl(),
                        $enclosure->getLength(),
                        $enclosure->getMimeType()
                    );
