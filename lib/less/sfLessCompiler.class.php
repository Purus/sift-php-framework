...
class sfLessCompiler extends lessc {
  ...
  public function construct()
  {
    ...
    $componentsPath = sfConfig::get('sf_less_components_path', 
            sfConfig::get('sf_web_dir'). '/css/_');    
    ...
    $adminWebDir = sfConfig::get('sf_admin_web_dir');
    ...
    $variables = array(
      ...
      '@sf_admin_web_dir' => sprintf('"%s"', $adminWebDir)
    );
  ...
  public function compileStylesheetIfNeeded($source)
  {
    ...
    // sift webdirectory 
    if(strpos($source, sfConfig::get('sf_admin_web_dir')) !== false)
    {
      $part   = str_replace(sfConfig::get('sf_admin_web_dir'), '', $source);
      $source = sfConfig::get('sf_web_dir') . sfConfig::get('sf_admin_web_dir') . $part;       
      ...
      if(!is_readable($source))
      {
        $source = sfConfig::get('sf_sift_data_dir') . DIRECTORY_SEPARATOR . 'web' . DIRECTORY_SEPARATOR . 
                'sf'. DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . $part; 
      }
    }
    else
    {
	...
	protected function findImport($url) 
  {
		foreach($this->importDir as $dir) 
    {
      ...
      $file = sprintf('%s.less', $dir . DIRECTORY_SEPARATOR . $url);
