...
class sfLessCompiler extends lessc implements sfIService
{
    ...
    public function construct()
    {
        ...
        $this->pathAliases = $this->dispatcher->filter(new sfEvent('less.compile.path_aliases'), $this->pathAliases)
            ->getReturnValue();
        ...
        $importDir = array(
            str_replace(
                DIRECTORY_SEPARATOR,
                '/',
                sfConfig::get('sf_less_import_path', sfConfig::get('sf_web_dir') . DIRECTORY_SEPARATOR . 'css')
            ),
    ...
    public function compileStylesheetIfNeeded($source)
    {
        ...
        try {
            $this->compileIfNeeded(
                $source,
                $this->options['web_cache_dir'] . '/' . $this->options['web_cache_dir_suffix'] . '/' . $cache
            );
        } catch (Exception $e) {
        ...
        return sprintf(
            '%s/cache/css/%s/%s',
            $this->getRelativeUrlRoot(),
            $this->options['web_cache_dir_suffix'],
            $cache
        );
    }
