...
class sfYamlParser
{
    ...
    public function parse($value)
    {
        ...
        while ($this->moveToNextLine()) {
            ...
            if (preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+?))?\s*$#u', $this->currentLine, $values)) {
                ...
                if (preg_match(
                    '#^(?P<key>' . sfYamlInline::REGEX_QUOTED_STRING . '|[^ \'"].*?) *\:(\s+(?P<value>.+?))?\s*$#u',
                    $this->currentLine,
                    $values
                )
                ) {
    ...
    protected function parseFoldedScalar($separator, $indicator = '', $indentation = 0)
    {
        ...
        if (!preg_match(
            '#^(?P<indent>' . ($indentation ? str_repeat(' ', $indentation) : ' +') . ')(?P<text>.*)$#u',
            $this->currentLine,
            $matches
        )
        ) {
        ...
        while ($this->currentLineNb + 1 < count($this->lines)) {
            ...
            if (preg_match(
                ...
                $text .= str_repeat(' ', $diff = strlen($matches['indent']) - strlen($textIndent))
                    . $matches['text'] . ($diff ? "\n" : $separator);
