...
class sfFileCache extends sfCache
{
    ...
    public function removePattern($pattern)
    {
        if (false !== strpos($pattern, '**')) {
            ...
            $paths = array();
            foreach (
                new RecursiveIteratorIterator(new sfSkipDotsRecursiveDirectoryIterator($this->getOption(
                    'cache_dir'
                ))) as $path
            ) {
            ...
            $paths = sfGlob::find(
                $this->getOption('cache_dir') . DIRECTORY_SEPARATOR . str_replace(
                    self::SEPARATOR,
                    DIRECTORY_SEPARATOR,
                    $pattern
    ...
    public function clean($mode = self::MODE_ALL)
    {
        ...
        $result = true;
        foreach (
            new RecursiveIteratorIterator(new sfSkipDotsRecursiveDirectoryIterator($this->getOption('cache_dir'))) as
            $file
        ) {
    ...
    {
        return $this->getOption('cache_dir') . DIRECTORY_SEPARATOR . str_replace(
            self::SEPARATOR,
            DIRECTORY_SEPARATOR,
            $key
        ) . $this->getOption('suffix');
