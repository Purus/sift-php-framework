...
class sfTesterResponse extends sfTester
{
    ...
    public function checkElement($selector, $value = true, $options = array())
    {
            ...
            if (true === $value) {
                ...
                if (is_int($value)) {
                    $this->tester->is(
                        count($values),
                        $value,
                        sprintf('response selector "%s" matches "%s" times', $selector, $value)
                    );
                } else {
                    if (preg_match('/^(!)?([^a-zA-Z0-9\\\\]).+?\\2[ims]?$/', $value, $match)) {
                        ...
                        if ($match[1] == '!') {
                            $this->tester->unlike(
                                ...
                                substr($value, 1),
                                sprintf(
                                    'response selector "%s" does not match regex "%s"',
                                    $selector,
                                    substr($value, 1)
                                )
                            );
                            ...
                            $this->tester->like(
                                @$values[$position],
                                $value,
                                sprintf('response selector "%s" matches regex "%s"', $selector, $value)
                            );
                        }
                    } else {
                        $position = isset($options['position']) ? $options['position'] : 0;
                        $this->tester->is(
                            @$values[$position],
                            $value,
                            sprintf('response selector "%s" matches "%s"', $selector, $value)
                        );
                    }
                }
            }
        ...
        if (isset($options['count'])) {
            $this->tester->is(
                count($values),
                $options['count'],
                sprintf('response selector "%s" matches "%s" times', $selector, $options['count'])
            );
        }
    ...
    public function checkForm($form, $selector = 'form')
    {
        ...
        $rendered = array();
        foreach (
            $this->domCssSelector->matchAll(sprintf('%1$s input, %1$s textarea, %1$s select', $selector))->getNodes() as
            $element
        ) {
            ...
            foreach ($dom->getElementsByTagName('*') as $element) {
                if (in_array($element->tagName, array('input', 'select', 'textarea'))) {
                    ...
                    $this->tester->ok(
                        false !== $pos,
                        sprintf(
                            'response includes "%s" form "%s" field - "%s %s[name=%s]"',
                            get_class($form),
                            $field,
                            $selector,
                            $element->tagName,
        ...
        if (preg_match('/(x|ht)ml/i', $this->response->getContentType())) {
            ...
            switch (pathinfo($checkDTD, PATHINFO_EXTENSION)) {
                ...
                case 'rnc':
                    ...
                    break;
                default:
                    $message = $dom->validateOnParse ? sprintf('response validates as "%s"', $dom->doctype->name)
            ...
            throw new LogicException(sprintf(
                'Unable to validate responses of content type "%s"',
            ...
            if (false !== $regex) {
                if ($mustMatch) {
                    if (preg_match($regex, $header)) {
                        ...
                        $this->tester->pass(
                            sprintf(
                                'response header "%s" matches "%s" (%s)',
                                $key,
                                $value,
                                $this->response->getHttpHeader($key)
                            )
                        );
                        break;
                    }
                    ...
                    if (preg_match($regex, $header)) {
                        ...
                        $this->tester->fail(
                            sprintf(
                                'response header "%s" does not match "%s" (%s)',
                                $key,
                                $value,
                                $this->response->getHttpHeader($key)
                            )
                ...
                $ok = true;
                $this->tester->pass(
                    sprintf('response header "%s" is "%s" (%s)', $key, $value, $this->response->getHttpHeader($key))
                );
                break;
            }
        }
        ...
        foreach ($this->response->getCookies() as $cookie) {
            if ($name == $cookie['name']) {
                if (null === $value) {
                    ...
                    $this->tester->ok(
                        $value == $cookie['value'],
                        sprintf('response sets cookie "%s" to "%s"', $name, $value)
                    );
                }
    ...
    public function matches($regex)
    {
        ...
        if ($match[1] == '!') {
            $this->tester->unlike(
                ...
                substr($regex, 1),
                sprintf('response content does not match regex "%s"', substr($regex, 1))
            );
        } else {
            $this->tester->like(
                $this->response->getContent(),
    ...
    public function isRedirected($boolean = true, $url = null)
    {
        if ($location = $this->response->getHttpHeader('Location')) {
            $boolean
                ? $this->tester->pass(sprintf('page redirected to "%s"', $location))
                : $this->tester->fail(
                sprintf('page redirected to "%s"', $location)
            );
            if ($url) {
                if ($url == $location) {
