...
class sfTesterMailer extends sfTester
{
        ...
        foreach ($this->logger->getMessages() as $message) {
            ...
            if ($to == key($email)) {
                ...
                if ($matches == $position) {
                    ...
                    $this->tester->pass(
                        sprintf('switch context to the message number "%s" sent to "%s"', $position, $messageEmail)
                    );
    ...
    public function checkBodyMultipart($value, $contentType = 'text/plain', $matchFirst = true)
    {
        ...
        if (!$tested) {
            $this->tester->fail(
                sprintf(
                    'there is no multipart for given contentType "%s" which should match "%s"',
                    $contentType,
                    $value
                )
            );
            ...
            if ($child->getFilename() == $attachmentName) {
                if ($contentType) {
                    if ($child->getHeaders()->get('content-type')->getValue() != $contentType) {
                        $this->tester->fail(
                            sprintf(
                                'attachment "%s" does not have the right content type "%s"',
                                $attachmentName,
                                $contentType
                            )
                        );
                        ...
                        $this->tester->pass(
                            sprintf(
                                'attachment "%s" does have the right content type "%s"',
                                $attachmentName,
                                $contentType
                            )
                        );
            ...
            if (false !== $regex) {
                if ($mustMatch) {
                    ...
                    if (preg_match($regex, $header)) {
                        $ok = true;
                        $this->tester->fail(
                            sprintf('email header "%s" does not match "%s" (%s)', $key, $value, $current)
                        );
                        break;
                    }
                }
