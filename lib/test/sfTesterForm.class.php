...
class sfTesterForm extends sfTester
{
    ...
    public function hasErrors($value = true)
    {
        ...
        if (is_int($value)) {
            $this->tester->is(
                count($this->form->getErrorSchema()),
                $value,
                sprintf('the submitted form has "%s" errors.', $value)
            );
        } else {
            $this->tester->is(
                $this->form->hasErrors(),
                $value,
                sprintf('the submitted form %s.', ($value) ? 'has some errors' : 'is valid')
            );
        }
    ...
    public function isError($field, $value = true)
    {
        ...
        if (false === $value) {
            ...
            if (true === $value) {
                ...
                if (is_int($value)) {
                    $this->tester->ok(
                        $error && count($error) == $value,
                        sprintf('the submitted form has %s "%s" error(s).', $value, $field)
                    );
                } else {
                    if (preg_match('/^(!)?([^a-zA-Z0-9\\\\]).+?\\2[ims]?$/', $value, $match)) {
                        if (!$error) {
                            ...
                            if ($match[1] == '!') {
                                $this->tester->unlike(
                                    ...
                                    substr($value, 1),
                                    sprintf(
                                        'the submitted form has a "%s" error that does not match "%s".',
                                        $field,
                                        $value
                                    )
                                );
                                ...
                                $this->tester->like(
                                    $error->getCode(),
                                    $value,
                                    sprintf('the submitted form has a "%s" error that matches "%s".', $field, $value)
                                );
                            }
                        }
                    } else {
                        if (!$error) {
                            ...
                            $this->tester->is(
                                $error->getCode(),
                                $value,
                                sprintf('the submitted form has a "%s" error (%s).', $field, $value)
                            );
                        }
                    }
                }
