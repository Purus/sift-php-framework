...
class sfTesterRequest extends sfTester
{
    ...
    public function isParameter($key, $value)
    {
        $this->tester->is(
            ...
            $value,
            sprintf('request parameter "%s" is "%s"', $key, $value)
        );
        ...
        if (preg_match('/^(!)?([^a-zA-Z0-9\\\\]).+?\\2[ims]?$/', $value, $match)) {
            if ($match[1] == '!') {
                $this->tester->unlike(
                    $_COOKIE[$name],
                    substr($value, 1),
                    sprintf('cookie "%s" content does not match regex "%s"', $name, $value)
                );
            } else {
                $this->tester->like(
                    $_COOKIE[$name],
                    $value,
                    sprintf('cookie "%s" content matches regex "%s"', $name, $value)
                );
            }
        } else {
