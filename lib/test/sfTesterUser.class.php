...
class sfTesterUser extends sfTester
{
    ...
    public function isAttribute($key, $value, $ns = null)
    {
        $this->tester->is(
            ...
            $value,
            sprintf('user attribute "%s" is "%s"', $key, $value)
        );
    ...
    {
        $this->tester->is(
            (string)$this->user->getFlash($key, null, $ignoreApplication),
            $value,
            sprintf('user flash "%s" is "%s"', $key, $value)
        );
    ...
    public function isAuthenticated($boolean = true)
    {
        $this->tester->is(
            ...
            $boolean,
            sprintf('user is %sauthenticated', $boolean ? '' : 'not ')
        );
    ...
    {
        $this->tester->is(
            $this->user->hasCredential($credentials, $useAnd),
            $boolean,
            sprintf('user has %sthe right credentials', $boolean ? '' : 'not ')
        );
