...
class sfRenderingFilter extends sfFilter
{
    ...
    protected function prepare()
    {
        ...
        $this->getContext()->getEventDispatcher()->notify(
            new sfEvent('response.pre_send', array(
                'response' => $response
            ))
        );
            ...
            if ($this->canCompress() && ($encoding = $this->getClientEncoding())) {
                ...
                if (mb_strlen($content, $this->getParameter('charset')) >
                    ...
                    try {
                        ...
                        $this->log(
                            'Compresed the output using "{encoding}". Compressed size {saving}%.',
                            sfILogger::INFO,
                            array(
                                'encoding' => $encoding,
                                'saving'   => ($total = strlen($response->getContent())) > 0 ? ((round(
                                            strlen($content) / $total,
                                            2
                                        ) * 100)) : 0
                            )
                        );
    ...
    protected function removeWhitespace($content)
    {
        try {
            ...
            $this->log(
                'Error while removing whitespace. {exception}',
                sfILogger::WARNING,
                array(
