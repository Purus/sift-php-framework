...
class sfCompressOutputFilter extends sfFilter
{
  ...
  protected function tidy($text)
  {
    ...
    if($log_enabled)
    {
      sfLogger::getInstance()->info('{myCompressOutputFilter} Cleaning up the response content');
    }
  ...
  protected function compress($content)
  {
    ...
    if(sfConfig::get('sf_compressed'))
    {
      ...
      {
        sfLogger::getInstance()->err('{myCompressOutputFilter} Compression setting sf_compressed should be turned off. With this setting the framework tries to compress everything (even images of generated by your app.).');
      }
      return $content;
    }
