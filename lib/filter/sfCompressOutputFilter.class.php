...
class sfCompressOutputFilter extends sfFilter
{
   ...
   * @author Converted to a Symfony filter by Mishal <mishal@mishal.cz>
  ...
  {
    // detect symfony compression flag
    // do nothing when comression is enabled
    if(sfConfig::get('sf_compressed'))
    {
      if(sfConfig::get('sf_logging_enabled'))
      {
        sfContext::getInstance()->getLogger()->err('{myCompressOutputFilter} Compression setting sf_compressed should be turned off. With this setting symfony tries to compress everything (even images of generated by your app.).');
      }
      return $content;
    }
