...
class sfBasicSecurityFilter extends sfSecurityFilter
{
    ...
    public function execute(sfFilterChain $filterChain)
    {
        ...
        if (
            (sfConfig::get('sf_login_module') == $context->getModuleName())
            ...
            if (sfConfig::get('sf_use_flash')) {
                ...
                $user->setFlash(
                    ...
                    __(
                        ...
                        array(),
                        sfConfig::get('sf_sift_data_dir') . '/i18n/catalogues/action'
                    )
                );
            }
    ...
    protected function handle401Ajax()
    {
        ...
        if ($loginRoute = $this->getLoginRoute()) {
            ...
            $response->setContent(
                sfJson::encode(
                    sfAjaxResult::createFromArray(
                        array(
                            'html' => __(
                                'This action requires you to be logged in and you are not. Redirect to login page?',
                                array(),
                                sfConfig::get('sf_sift_data_dir') . '/i18n/catalogues/action'
                            ),
