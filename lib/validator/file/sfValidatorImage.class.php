...
class sfValidatorImage extends sfValidatorFile
{
    ...
    protected function configure($options = array(), $messages = array())
    {
        ...
        $this->setMessage(
            'required',
            isset($messages['required']) ? $messages['required'] : 'Select an image to be uploaded.'
        );
        $this->setMessage('invalid', isset($messages['invalid']) ? $messages['invalid'] : 'Invalid file uploaded.');

        $this->addMessage(
            'too_small',
            'Image dimensions %width%x%height%px are too small (minimum is %min_width%x%min_height%px).'
        );
        $this->addMessage(
            'too_large',
            'Image dimensions %width%x%height%px are too large (maximum is %max_width%x%max_height%px).'
        );
    }
    ...
    protected function doClean($value)
    {
        ...
        if ($result instanceof $class) {
            ...
            if ($dimensions[0] < $this->getOption('min_width') || $dimensions[1] < $this->getOption('min_height')) {
        ...
        if ($this->getOption('persistent')) {
            ...
            sfCore::getEventDispatcher()->notify(
                new sfEvent('form.validator.image.persistent.post_save', array(
                    'info'            => $info,
                    'file'            => $result,
                    'persistent_info' => $persistentUploadInfo
                ))
            );
        }
        ...
        if (!file_exists($dir)) {
            if (!@mkdir($dir)) {
                throw new sfException(sprintf(
                    'Unable to create "$dir" check permissions of parent directory or create this directory manually and make sure it is writable by the web server',
                    $dir
                ));
            }
        }
