...
class sfValidatorTime extends sfValidatorBase
{
    ...
    protected function configure($options = array(), $messages = array())
    {
        ...
        $this->setMessage('required', 'Time is required.');
        $this->setMessage('invalid', '"%value%" is invalid time value.');
            ...
            if ($regex = $this->getOption('time_format')) {
                if (!preg_match($regex, $value, $match)) {
                    throw new sfValidatorError($this, 'bad_format', array(
                        'value'       => $value,
                        'time_format' => $this->getOption(
                                'time_format_error'
                            ) ? $this->getOption('time_format_error') : $this->getOption('time_format')
                    ));
                }
    ...
    protected function convertTimeArrayToTimestamp($value)
    {
        ...
        if (
            $this->isValueSet($value, 'second')
            && (!$this->isValueSet($value, 'minute')
                || !$this->isValueSet(
                    $value,
                    'hour'
                ))
