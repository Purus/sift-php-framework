...
/**
 * sfValidatorDateTimeRange validates a range of date time. It also converts the input values to valid dates.
 *
 * @package    Sift
...
class sfValidatorDateTimeRange extends sfValidatorBase
{
    ...
    protected function doClean($value)
    {
        ...
        $value[$fromField] = $this->getOption('from_date')->clean(
            isset($value[$fromField]) ? $value[$fromField] : null
        );
        $value[$toField] = $this->getOption('to_date')->clean(isset($value[$toField]) ? $value[$toField] : null);

        if ($value[$fromField] && $value[$toField]) {
            $v
                = new sfValidatorSchemaCompare($fromField, sfValidatorSchemaCompare::LESS_THAN_EQUAL, $toField, array('throw_global_error' => true), array(
                'invalid' => $this->getMessage(
                        'invalid'
                    )
            ));
            $v->clean($value);
