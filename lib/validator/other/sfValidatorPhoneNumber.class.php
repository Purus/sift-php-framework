...
class sfValidatorPhoneNumber extends sfValidatorBase
{
     ...
     * @var array
     */
    public static $searchReplaceMapping
        = array(
            '++'   => '+',
            '+'    => '',
            '(0)'  => '',
     ...
     * @var type
     */
    public static $searchKeypadMapping
        = array(
            'a' => '2',
            'b' => '2',
            'c' => '2',
    ...
    public function __construct($options = array(), $messages = array())
    {
        ...
        if (!$this->getOption('countries') && !$this->getOption('pattern')) {
            throw new InvalidArgumentException(sprintf(
                'Please specify an array of countries or PCRE regular expression pattern for your registered %s validator.',
                get_class($this)
            ));
        }
    ...
    public static function normalizePhoneNumber($number, $country)
    {
        ...
        elseif (preg_match(
            sprintf('/^%s%s/', $format['iprefix'], $format['code']),
            $number,
            $matches,
            PREG_OFFSET_CAPTURE
        )
    ...
    public function getJavascriptValidationMessages()
    {
        ...
        $messages[sfFormJavascriptValidation::CUSTOM_CALLBACK] = sfFormJavascriptValidation::fixValidationMessage(
            $this,
            'invalid'
        );
