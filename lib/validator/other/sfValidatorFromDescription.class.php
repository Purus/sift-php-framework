    ...
    protected function tokenize($string)
    {
        ...
        while ($i < $len) {
            if (preg_match('/^([a-z0-9_\-]+)\s*(<=|>=|<|>|==|!=)/i', substr($string, $i), $match)) {
                ...
                $tokens[] = new sfValidatorFDToken('sfValidatorSchemaCompare', array(
                    $leftField,
                    $operator,
                    $rightField,
                    $arguments[0],
                ...
                if (preg_match('/^(and|or)/i', substr($string, $i), $match)) {
                    ...
                    if (preg_match('/^(?:([a-z0-9_\-]+)\:)?([a-z0-9_\-]+)/i', substr($string, $i), $match)) {
                        ...
                        $arguments = $this->parseArguments($string, $i);
                        $token = new sfValidatorFDToken($class, array(
                            $arguments[0],
                            isset($arguments[1]) ? $arguments[1] : array()
                        ));
                        if ($match[1]) {
            ...
            switch (get_class($token)) {
                ...
                case 'sfValidatorFDTokenOperator':
                    while (count($operatorStack)
                        && $precedences[$token->__toString()] <= $precedences[$operatorStack[
                        count($operatorStack) - 1]->__toString()]) {
                        $outputStack[] = array_pop($operatorStack);
                    }
                    $operatorStack[] = $token;
                    break;
...
class sfValidatorFDTokenOperator
{
    ...
    public function asPhp($tokenLeft, $tokenRight)
    {
        return sprintf(
            ...
            && in_array(
                get_class($tokenLeft),
                array('sfValidatorFDToken', 'sfValidatorFDTokenFilter')
            ) ? $tokenLeft->asPhp() : $tokenLeft,
            is_object($tokenRight)
            && in_array(
                get_class($tokenRight),
                array('sfValidatorFDToken', 'sfValidatorFDTokenFilter')
            ) ? $tokenRight->asPhp() : $tokenRight,
            implode(', ', array_map(create_function('$a', 'return var_export($a, true);'), $this->arguments))
        );
    }
    ...
    public function getValidator($tokenLeft, $tokenRight)
    {
        ...
        $validators = array(
            in_array(get_class($tokenLeft), array('sfValidatorFDToken', 'sfValidatorFDTokenFilter'))
                ? $tokenLeft->getValidator() : $tokenLeft,
            in_array(get_class($tokenRight), array('sfValidatorFDToken', 'sfValidatorFDTokenFilter'))
                ? $tokenRight->getValidator() : $tokenRight,
        );
...
/**
 * sfValidatorFDTokenRightBracker
 *
 * @package    Sift
 * @subpackage validator
 */
