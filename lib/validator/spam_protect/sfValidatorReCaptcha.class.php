...
class sfValidatorReCaptcha extends sfValidatorBase
{
    ...
    protected function doClean($value)
    {
        ...
        if (true !== ($answer = $this->check(
                array(
                    'privatekey' => $this->getOption('private_key'),
                    'remoteip'   => $this->getOption('remote_addr') ? $this->getOption('remote_addr')
                            : $_SERVER['REMOTE_ADDR'],
                    'challenge'  => $challenge,
                    'response'   => $response,
                )
    ...
    protected function check($parameters)
    {
        if (false === ($fs = @fsockopen(
                ...
                $this->getOption('server_port'),
                $errno,
                $errstr,
                $this->getOption('server_timeout')
            ))
        ) {
        ...
        fwrite(
            ...
            sprintf(
                ...
                "Content-Length: %d\r\n" .
                "User-Agent: reCAPTCHA/SiftPHPFramework\r\n" .
                "\r\n%s",
