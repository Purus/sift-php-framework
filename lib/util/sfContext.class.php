...
class sfContext
{
    ...
    public function __construct(sfApplication $application, sfServiceContainer $serviceContainer = null)
    {
        ...
        $this->application->getShutdownScheduler()->register(
            array($this, 'shutdown'),
            array(),
            sfShutdownScheduler::LOW_PRIORITY
        );

        $this->application->getEventDispatcher()->notify(
            new sfEvent('context.load_factories', array(
                'context' => $this
            ))
        );
    ...
    public static function createInstance(sfApplication $application, $name = null, $class = __CLASS__)
    {
        ...
        $instance->getApplication()->getEventDispatcher()->notify(
            new sfEvent('context.instance_created', array(
                'context' => $instance
            ))
        );
    ...
    public function retrieveObjects($class, $peerMethod, $options = array())
    {
        ...
        if (!class_exists($retrievingClass)) {
            throw new InvalidArgumentException(sprintf(
                'The data retriever class "%s" does not exist'
            ), $retrievingClass);
        }
