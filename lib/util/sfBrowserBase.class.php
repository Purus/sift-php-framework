 ...
 *
 * It implements features that is independent from the Sift controllers.
 *
 * @package    Sift
 * @subpackage util
 */
abstract class sfBrowserBase
{
    ...
    public function call($uri, $method = 'get', $parameters = array(), $changeStack = true)
    {
        ...
        if ($this->stackPosition >= 1) {
            $_SERVER['HTTP_REFERER'] = sprintf(
                'http%s://%s%s',
                isset($this->defaultServerArray['HTTPS']) ? 's' : '',
                $this->hostname,
                $this->stack[$this->stackPosition - 1]['uri']
            );
        }
    ...
    public function doSelect($name, $selected)
    {
        ...
        if ($element = $xpath->query(
            sprintf(
                '//input[(@type="radio" or @type="checkbox") and (.="%s" or @id="%s" or @name="%s")]',
                $name,
                $name,
                $name
            )
            ...
            if ($selected) {
                if ($element->getAttribute('type') == 'radio') {
                    ...
                    foreach (
                        $xpath->query(
                            sprintf('//input[@type="radio" and @name="%s"]', $element->getAttribute('name'))
                        ) as $radio
                    ) {
    ...
    public function doClick($name, $arguments = array(), $options = array())
    {
        ...
        $query .= sprintf(
            '|//input[((@type="submit" or @type="button") and @value="%s") or (@type="image" and @alt="%s")]',
            $name,
            $name
        );
        ...
        if (!$item = $list->item($position)) {
            throw new InvalidArgumentException(sprintf(
                'Cannot find the "%s" link or button (position %d).',
                $name,
                $position + 1
            ));
        }
    ...
    public function doClickCssSelector($selector, $arguments = array(), $options = array())
    {
        ...
        if (isset($elements[$position])) {
            ...
            throw new InvalidArgumentException(sprintf(
                'Could not find the element "%s" (position %d) in the current DOM.',
                $selector,
                $position + 1
            ));
        }
    ...
    public function doClickElement(DOMElement $item, $arguments = array(), $options = array())
    {
        ...
        $method = strtolower(
            isset($options['method'])
                ? $options['method'] : ($item->getAttribute('method') ? $item->getAttribute(
                'method'
            ) : 'get')
        );
        ...
        foreach ($xpath->query('descendant::input | descendant::textarea | descendant::select', $item) as $element) {
            ...
            if ($nodeName == 'input'
                ...
                if ($nodeName == 'input' && $element->getAttribute('type') == 'file') {
                    $filename = array_key_exists($elementName, $arguments) ? $arguments[$elementName]
                        : sfToolkit::getArrayValueForPath($arguments, $elementName, '');
