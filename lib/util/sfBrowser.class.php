...
class sfBrowser extends sfBrowserBase
{
    ...
    public function getContext($forceReload = false)
    {
        if (null === $this->context || $forceReload) {
            ...
            $application = sfCore::getApplication(
                $currentApplication->getOption('sf_app'),
                $currentApplication->getEnvironment(),
                $currentApplication->isDebug(),
                true
            );

            // connect listeners
            $application->getEventDispatcher()->connect(
                'application.throw_exception',
                array($this, 'listenToException')
            );
