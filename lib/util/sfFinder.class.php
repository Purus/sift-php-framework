...
class sfFinder
{
     ...
     * @var array
     */
    public static $versionControlIgnores
        = array(
            '.svn',
            '_svn',
            'CVS',
            ...
            if ($type === 'any') {
                ...
                if ($type === 'file') {
                    ...
                    throw new InvalidArgumentException(sprintf(
                        'Invalid type "%s" given. Valid types are: "file", "dir" or "any".',
                        $type
                    ));
                }
            }
        ...
        for ($i = 0, $count = count($args); $i < $count; $i++) {
            if (is_array($args[$i]) && !method_exists($args[$i][0], $args[$i][1])) {
                throw new sfException(sprintf(
                    'The method "%s" does not exist for object "%s".',
                    $args[$i][1],
                    $args[$i][0]
                ));
            }
