...
class sfHtml
{
    ...
    public static function attributesToHtml($attributes)
    {
        return implode(
            '',
            array_map(array('sfHtml', 'attributesToHtmlCallback'), array_keys($attributes), array_values($attributes))
        );
    }
    ...
    {
        return self::fixDoubleEscape(
            htmlspecialchars(!is_array($value) ? (string)$value : null, ENT_COMPAT, self::getCharset())
        );
    }
