...
/**
 * sfClassManipulator manipulates PHP class code.
 *
 * @package    Sift
 * @subpackage util
 */
class sfClassManipulator
{
  ...
  /**
   * Appends a method to the PHP code
   *
   * @param string $name
   * @param string $code
   */
  public function appendMethod($name, $arguments, $code, $access = array(), $indent = 2)
  {
    $method = array();

    $method[] = sprintf('%s function %s(%s)',
            join(' ', $access), $name, join(', ', $arguments));
    ...
    // replace last }
    $this->code = preg_replace('/(\}+)\s?(\?>)?\s?$/',  $method . "\n}", $this->code);
  }
