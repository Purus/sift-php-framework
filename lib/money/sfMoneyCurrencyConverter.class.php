...
class sfMoneyCurrencyConverter implements sfIMoneyCurrencyValue
{
    ...
    public function __construct($conversionRate, $sourceCurrency, $targetCurrency)
    {
        ...
        if (strpos($conversionRate, '/') !== false) {
            ...
            if ($b == 0) {
                throw new InvalidArgumentException(sprintf(
                    'Invalid conversion rate "%s". The conversion rate divides by zero.',
                    $conversionRate
                ));
            }
        ...
        $this->sourceCurrency = is_string($sourceCurrency) ? sfMoneyCurrency::getInstance($sourceCurrency)
            : $sourceCurrency;
        $this->targetCurrency = is_string($targetCurrency) ? sfMoneyCurrency::getInstance($targetCurrency)
            : $targetCurrency;
    }
    ...
    public static function createFromIso($iso)
    {
        $currency = '([A-Z]{2,3})';
        $ratio
            = '((\d+/[0-9]*\.?[0-9]+)|([0-9]*\.?[0-9]+))'; // @see http://www.regular-expressions.info/floatingpoint.html
    ...
    public function setMoney(sfIMoneyCurrencyValue $money)
    {
        if (!$this->isInSameCurrency($money)) {
            throw new InvalidArgumentException(
                sprintf(
                    'The money value "%s" is in an incorrect currency "%s" for this converter. Expected currency: "%s".',
                    $money->getAmount(),
    ...
    {
        $result = sfMath::multiply(
            $this->money->getAmount(),
            $this->conversionRate,
            sfMoneyCurrencyValue::$calculationPrecision
        );
