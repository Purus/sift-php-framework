...
abstract class sfAction extends sfComponent
{
    ...
    public function initialize(sfContext $context)
    {
        ...
        if ($file = sfConfigCache::getInstance()->checkConfig(
            sfConfig::get('sf_app_module_dir_name') . '/' . $this->getModuleName() . '/' . sfConfig::get(
    ...
    {
        $this->logMessage(
            '{sfAction} Forwarding to {module_action}',
            sfILogger::INFO,
            array(
    ...
    public function redirect($url, $statusCode = 302, $getParameters = array())
    {
        ...
        $this->logMessage(
            '{sfAction} Redirect to "{url}", code: {code}',
            sfILogger::INFO,
            array(
                'url'  => $url,
                'code' => $statusCode
    ...
    public function getCredential()
    {
        ...
        if (isset($this->security[$actionName]['credentials'])) {
            ...
            if (isset($this->security['all']['credentials'])) {
    ...
    public function setTemplate($name)
    {
        ...
        $this->getResponse()->setParameter(
            $this->getModuleName() . '_' . $this->getActionName() . '_template',
            $name,
            'sift/action/view'
        );
    }
    ...
    {
        return $this->getResponse()->getParameter(
            $this->getModuleName() . '_' . $this->getActionName() . '_template',
            null,
            'sift/action/view'
        );
    ...
    {
        $this->logMessage(
            '{sfAction} Change layout to "{layout}"',
            sfILogger::INFO,
            array(
                'layout' => $name
            )
        ...
        $this->getResponse()->setParameter(
            $this->getModuleName() . '_' . $this->getActionName() . '_layout',
            $name,
            'sift/action/view'
        );
    }
    ...
    {
        $this->logMessage(
            '{sfAction} getMailBody() is deprecated. Use $mail_message->setBodyFromPartial() instead.',
            sfILogger::ERROR
        );
        ...
        if (!in_array($type, array('plain', 'html'))) {
            throw new sfConfigurationException(sprintf(
                'Invalid email type passed ("%s"). Valid types are "plain" or "html".',
                $type
            ));
        }
