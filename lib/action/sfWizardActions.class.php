...
class sfWizardActions extends myActions
{
    ...
    public function preExecute()
    {
        ...
        try {
            ...
            myWizardForm::setStorageNamespace(
    ...
    protected function setStepValues($values, $overwrite = false)
    {
        if ($overwrite) {
            $this->getUser()->getAttributeHolder()->removeNamespace(
                $this->getWizardStorageNamespace() . '/' . $this->step
            );

            return $this->getUser()->getAttributeHolder()->add(
                $values,
                $this->getWizardStorageNamespace() . '/' . $this->step
            );
        } else {
    ...
    protected function stepIsValidated($step, $boolean = null)
    {
        ...
        if (!is_null($boolean)) {
            $this->getUser()->getAttributeHolder()->add(
                array('is_valid' => $boolean),
                $this->getWizardStorageNamespace() . '/' . $step
            );
