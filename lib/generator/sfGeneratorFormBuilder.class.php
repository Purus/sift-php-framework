...
//        )
    ...
    {
        $widgetClass = $this->getOptionFor(
            'widget.class',
            sfGeneratorField::TYPE_STRING,
            $context,
        ...
        $validatorClass = $this->getOptionFor(
            'validator.class',
            sfGeneratorField::TYPE_STRING,
            $context,
            'sfValidatorString'
        ...
        $validatorMessages = $this->getOptionFor(
            'validator.messages',
            sfGeneratorField::TYPE_STRING,
            $context,
            array()
        ...
        } elseif ($field->isRegularExpression() && ($regexp = $field->getRegularExpression())) {
        ...
        switch ($context) {
            case sfGenerator::CONTEXT_FILTER:
                ...
                return array(
                    $this->getOptionFor(
                        'widget.class',
                        sfGeneratorField::TYPE_BOOLEAN,
                        $context,
                        'sfWidgetFormTrilean',
                        false
                    ),
                    array_merge(
                        $this->getOptionFor('widget.options', sfGeneratorField::TYPE_BOOLEAN, $context, array(), false),
                        array(
        ...
        switch ($context) {
            ...
            case sfGenerator::CONTEXT_FILTER:
                ...
                return array(
                    $this->getOptionFor(
                        'widget.class',
                        sfGeneratorField::TYPE_DATE,
                        $context,
                        'sfWidgetFormDateFilter',
                        false
                    ),
                    array_merge(
                        $this->getOptionFor('widget.options', sfGeneratorField::TYPE_DATE, $context, array(), false),
                        array(
                            'from' => new sfPhpExpression(sprintf(
                                    'new %s(%s, %s)',
                                    $widgetClass,
                                    $this->varExport($widgetOptions),
                                    $this->varExport($widgetAttributes)
                                )),
                            'to'   => new sfPhpExpression(sprintf(
                                    'new %s(%s, %s)',
                                    $widgetClass,
                                    $this->varExport($widgetOptions),
                                    $this->varExport($widgetAttributes)
    ...
    {
        $widgetClass = $this->getOption(
            sprintf('%s.widget.class', sfGeneratorField::TYPE_TIMESTAMP),
            'sfWidgetFormDateTime'
        );
        $widgetOptions = $this->getOption(sprintf('%s.widget.options', sfGeneratorField::TYPE_TIMESTAMP), array());
        $widgetAttributes = $this->getOption(
            sprintf('%s.widget.attributes', sfGeneratorField::TYPE_TIMESTAMP),
            array()
        );

        $validatorClass = $this->getOptionFor(
            'validator.class',
            sfGeneratorField::TYPE_TIMESTAMP,
            $context,
            'sfValidatorDateTime'
        );
        // add validation options
        $validatorOptions = $this->addValidationOptions(
            $field,
            $context,
                ...
                return array(
                    ...
                    array_merge(
                        ...
                        array(
                            'from'       => new sfPhpExpression(sprintf(
                                    'new %s(%s, %s)',
                                    $this->getOption('timestamp.widget.class', 'sfWidgetFormDateTime'),
                                    $this->varExport($widgetOptions),
                                    $this->varExport($widgetAttributes)
                                )),
                            'to'         => new sfPhpExpression(sprintf(
        ...
        switch ($context) {
            ...
            case sfGenerator::CONTEXT_FILTER:
                return array(
                    $this->getOptionFor(
                        'widget.class',
                        sfGeneratorField::TYPE_TIMEP,
                        $context,
                        'sfWidgetFormTimeFilter',
                        false
                    ),
                    array_merge(
                        $this->getOptionFor('widget.options', sfGeneratorField::TYPE_TIME, $context, array(), false),
                        array(
                            ...
                            'from' => new sfPhpExpression(sprintf(
                                    'new %s(%s, %s)',
                                    $this->getOption('time.widget.class', 'sfWidgetFormTime'),
                                    $this->varExport($widgetOptions),
                                    $this->varExport($widgetAttributes)
                                )),
                            'to'   => new sfPhpExpression(sprintf(
                    ...
                    $this->getOptionFor(
                        'validator.class',
                        sfGeneratorField::TYPE_TIME,
                        $context,
                        'sfValidatorTimeRange',
                        false
                    ),
    ...
    protected function getWidgetAndValidatorEnum(sfIGeneratorField $field, $context)
    {
        ...
        return array(
            ...
            array_merge(
                $this->getOptionFor('widget.options', sfGeneratorField::TYPE_ENUM, $context, array()),
                array(
            ...
            $this->addValidationOptions(
                ...
                $context,
                array_merge(
                    $this->getOptionFor('validator.options', sfGeneratorField::TYPE_ENUM, $context, array()),
                    array(
                        'choices' => $field->getValues()
                    )
                )
    ...
    protected function getWidgetAndValidatorPartial(sfIGeneratorField $field, $context)
    {
        return array(
            ...
            array_merge(
                $this->getOptionFor('widget.options', sfGeneratorField::TYPE_PARTIAL, $context, array()),
                array(
    ...
    protected function getWidgetAndValidatorComponent(sfIGeneratorField $field, $context)
    {
        return array(
            ...
            array_merge(
                $this->getOptionFor(
                    'widget.options',
                    sfGeneratorField::TYPE_COMPONENT,
                    $context,
                    array()
                ),
                array(
    ...
    protected function getOptionFor($key, $fieldType, $context, $default, $fallback = true)
    {
        if ($fallback) {
            return $this->getOption(
                sprintf('%s.%s.%s', $context, $fieldType, $key),
