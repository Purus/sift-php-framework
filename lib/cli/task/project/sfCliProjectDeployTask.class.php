...
class sfCliProjectDeployTask extends sfCliBaseTask
{
    ...
    protected function configure()
    {
        ...
        $rsyncExclude = file_exists($rsyncExcludeFile) ? trim(file_get_contents($rsyncExcludeFile))
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        if (!isset($properties[$env])) {
            throw new sfCliCommandException(sprintf(
                'You must define the configuration for server "%s" in config/properties.ini',
                $env
            ));
        }
