...
abstract class sfCliBaseTask extends sfCliCommandApplicationTask
{
    ...
    protected function doRun(sfCliCommandManager $commandManager, $options)
    {
        $event = $this->dispatcher->filter(
            new sfEvent('cli_task.filter_options', array(
        ...
        $event = new sfEvent('command.pre_command', array(
            'task'      => &$this,
        ...
        if ($requiresApplication && null === $this->application) {
            $application = $commandManager->getArgumentSet()->hasArgument('application')
                ...
                :
                ($commandManager->getOptionSet()->hasOption('application') ? $commandManager->getOptionValue(
                    'application'
                ) : null);
    ...
    public function setupDatabases()
    {
        if (!isset($this->databases)) {
            ...
            if ($appConfigDir = $this->environment->get('sf_app_config_dir')) {
                if ($file = is_readable(
                    $appConfigDir . '/' . $this->environment->get('sf_app_config_dir_name') . 'databases.yml'
                )
                ) {
                    $files[] = $file;
                }
    ...
    {
        if (count(
            $dirs = sfFinder::type('dir')->maxDepth(0)->followLink()->relative()->in(
    ...
    protected function reloadAutoload()
    {
        $this->initializeAutoload(
            $this->application ?
                $this->application : $this->commandApplication->getProject(),
            true
        );
    }
