...
EOF;
    ...
    public function execute($arguments = array(), $options = array())
    {
        ...
        if ($isPlugin) {
            $this->logSection(
                $this->getFullName(),
                sprintf('Find non "i18n ready" strings in the "%s" plugin', $application)
            );
        } else {
            $this->logSection(
                $this->getFullName(),
                sprintf('Find non "i18n ready" strings in the "%s" application', $application)
            );
        }
        ...
        foreach ($strings as $template => $messages) {
            ...
            $this->logSection(
                ...
                sprintf(
                    'Strings in "%s"',
                    str_replace(
                        str_replace(DIRECTORY_SEPARATOR, '/', $this->environment->get('sf_root_dir') . '/'),
                        '',
                        $template
                    )
                ),
