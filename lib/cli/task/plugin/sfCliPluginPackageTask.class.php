...
EOF;
        ...
        if ($extension = $options['package_file']) {
            if (is_string($extension)) {
                $this->getFilesystem()->copy(
                    $this->pluginDir . '/package.xml',
                    $this->pluginDir . '/package.xml' . $extension,
                    array('override' => true)
                );
            }
    ...
    protected function generatePackageFile(array $arguments, array $options)
    {
        ...
        if (false !== strpos($template, '##SUMMARY##')) {
            $tokens['SUMMARY'] = $this->askAndValidate(
                'Summarize your plugin in one line:',
                ...
                array(
                    'value' => htmlspecialchars(
                        $this->getProjectProperty('author'),
                        ENT_QUOTES,
                        $this->environment->get('sf_charset', 'UTF-8')
                    ),
                )
            );
        }
        ...
        if (false !== strpos($template, '##LEAD_NAME##')) {
            ...
            $tokens['LEAD_NAME'] = $this->askAndValidate(
                'Lead developer name:',
                $validator,
                array(
                    'value' => htmlspecialchars(
        ...
        if (false !== strpos($template, '##LEAD_EMAIL##')) {
            ...
            $tokens['LEAD_EMAIL'] = $this->askAndValidate(
        ...
        if (false !== strpos($template, '##LEAD_USERNAME##')) {
            ...
            $tokens['LEAD_USERNAME'] = $this->askAndValidate(
                'Lead developer username:',
                $validator,
                array(
                    'value' => htmlspecialchars(
                        $this->getProjectProperty('username'),
                        ENT_QUOTES,
                        $this->environment->get('sf_charset', 'UTF-8')
                    ),
                )
        ...
        if (false !== strpos($template, '##PLUGIN_VERSION##')) {
            ...
            $tokens['PLUGIN_VERSION'] = $this->askAndValidate(
                'Plugin version number (i.e. "1.0.5"):',
                $validator,
                array('value' => $options['plugin-version'])
            );

            // set api version based on plugin version
            $tokens['API_VERSION'] = version_compare($tokens['PLUGIN_VERSION'], '0.1.0', '>') ?
                join('.', array_slice(explode('.', $tokens['PLUGIN_VERSION']), 0, 2)) . '.0'
        ...
        if (false !== strpos($template, '##STABILITY##')) {
            ...
            $tokens['STABILITY'] = $this->askAndValidate(
                'Plugin stability:',
                $validator,
                array('value' => $options['plugin-stability'])
            );
        }

        if (false !== strpos($template, '##CHANNEL##')) {
            $tokens['CHANNEL'] = $this->askAndValidate(
                'Plugin channel:',
                new sfValidatorCallback(array(
                    'required' => true,
                    'callback' => create_function('$a, $b', 'return htmlspecialchars($b, ENT_QUOTES, \'UTF-8\');'),
