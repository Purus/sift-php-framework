...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        if (!isset($config[$environment])
            ...
            if (is_array($config[$environment][$pluginName]) && isset($config[$environment][$pluginName]['disabled'])
                && $config[$environment][$pluginName]['disabled']
            ) {
                $config[$environment][$pluginName] = array_merge(
                    (array)$config[$environment][$pluginName],
                    array(
                        'disabled' => false
                    )
                );
        ...
        $skeleton = file_get_contents(
            $this->environment->get('sf_sift_data_dir') . '/skeleton/project/config/plugins.yml'
        );
