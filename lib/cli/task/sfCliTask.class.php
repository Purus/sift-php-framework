...
abstract class sfCliTask
{
     ...
     *
     * @param sfCliCommandManager $commandManager An sfCliCommandManager instance
     * @param mixed               $options        The command line options
     *
     * @return integer 0 if everything went fine, or an error code
     */
    public function runFromCli(sfCliCommandManager $commandManager, $options = null)
    {
    ...
    {
        $commandManager = new sfCliCommandManager(new sfCliCommandArgumentSet($this->getArguments(
        )), new sfCliCommandOptionSet($this->getOptions()));
        ...
        foreach ($options as $name => $value) {
            if (is_string($name)) {
                if (false === $value || null === $value
                    || (isset($indexedOptions[$name])
                        && $indexedOptions[$name]->isArray()
                        && !$value)
                ) {
                    unset($options[$name]);
                    continue;
                ...
                $value = true === $value
                    ...
                    : sprintf(
                        '%s=%s',
                        $name,
                        isset($indexedOptions[$name]) && $indexedOptions[$name]->isArray() ? join(
                            ' --' . $name . '=',
                            (array)$value
                        ) : $value
                    );
        ...
        return $this->doRun(
            $commandManager,
            is_string($arguments) ? $arguments : implode(' ', array_merge($arguments, $options))
        );
    }
     ...
     *
     * @return sfCliCommandArgument An array of sfCliCommandArgument objects.
     */
    ...
    {
        $this->arguments[] = new sfCliCommandArgument($name, $mode, $help, $default);
    }

    /**
     * Returns the options objects.
     *
     * @return sfCliCommandOption An array of sfCliCommandOption objects.
     */
     ...
     * Add an option.
     *
     * This method always use the sfCliCommandOption class to create an option.
     *
     * @see sfCliCommandOption::__construct()
     */
    public function addOption($name, $shortcut = null, $mode = null, $help = '', $default = null)
    {
        $this->options[] = new sfCliCommandOption($name, $shortcut, $mode, $help, $default);
    }
    ...
    public function getSynopsis()
    {
        ...
        foreach ($this->getOptions() as $option) {
            ...
            $options[] = sprintf(
                '[' . ($option->isParameterRequired()
                    ? '%s--%s="..."' : ($option->isParameterOptional() ? '%s--%s[="..."]' : '%s--%s')) . ']',
                $shortcut,
                $option->getName()
            );
        }
    ...
    protected function process(sfCliCommandManager $commandManager, $options)
    {
        ...
        if (!$commandManager->isValid()) {
            throw new sfCliCommandArgumentsException(sprintf(
    ...
    protected function doRun(sfCliCommandManager $commandManager, $options)
    {
        ...
        $this->dispatcher->notify(new sfEvent('cli_task.post_execute', array('task' => $this)));
    ...
    {
        $this->logger->log($this->formatter->formatSection($section, $message, $size, $style));
    }
