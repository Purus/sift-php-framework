...
class sfCliTestUnitTask extends sfCliTestBaseTask
{
    ...
    protected function configure()
    {
        $this->addArguments(
            array(
                new sfCliCommandArgument('name',
                    sfCliCommandArgument::OPTIONAL | sfCliCommandArgument::IS_ARRAY, 'The test name'),
            )
        );
    ...
    protected function execute($arguments = array(), $options = array())
    {
        if (count($arguments['name'])) {
            ...
            foreach ($arguments['name'] as $name) {
                ...
                $files = array_merge(
                    $files,
                    $finder->in($this->environment->get('sf_test_dir') . '/unit/' . dirname($name))
                );
            }
