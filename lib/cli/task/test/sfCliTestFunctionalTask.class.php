    ...
    protected function configure()
    {
        $this->addArguments(
            array(
                ...
                new sfCliCommandArgument('controller',
                    sfCliCommandArgument::OPTIONAL | sfCliCommandArgument::IS_ARRAY, 'The controller name'),
            )
        );
        ...
        if (count($arguments['controller'])) {
            ...
            foreach ($arguments['controller'] as $controller) {
                ...
                $files = array_merge(
                    $files,
                    $finder->in(
                        $this->environment->get('sf_test_dir') . '/functional/' . $app . '/' . dirname($controller)
                    )
                );
            }
