...
EOF;
    ...
    protected function outputAsText(sfCliTask $task)
    {
        ...
        $messages[] = $this->formatter->format(
                sprintf(
                    ' ' . $task->getSynopsis(),
                    null === $this->commandApplication ? '' : $this->commandApplication->getScriptName()
                )
            ) . "\n";
        ...
        if ($task->getAliases()) {
            $messages[] = $this->formatter->format('Aliases:', 'COMMENT') . ' ' . $this->formatter->format(
        ...
        if ($task->getArguments()) {
            ...
            foreach ($task->getArguments() as $argument) {
                $default = null !== $argument->getDefault()
                && (!is_array($argument->getDefault())
                    ...
                    sprintf(
                        ...
                        is_array($argument->getDefault()) ? str_replace(
                            "\n",
                            '',
                            print_r($argument->getDefault(), true)
                        ) : $argument->getDefault()
                    ),
                    'COMMENT'
                ) : '';
                $messages[] = sprintf(
                    " %-${max}s %s%s",
                    $this->formatter->format($argument->getName(), 'INFO'),
                    $argument->getHelp(),
                    $default
                );
            }
        ...
        if ($task->getOptions()) {
            ...
            foreach ($task->getOptions() as $option) {
                $default = $option->acceptParameter() && null !== $option->getDefault()
                && (!is_array($option->getDefault())
                    || count($option->getDefault())) ? $this->formatter->format(
                    sprintf(
                        ' (default: %s)',
