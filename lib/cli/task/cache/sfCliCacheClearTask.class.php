...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        $apps
            =
            null === $options['app'] ? $dirFinder->in($this->environment->get('sf_apps_dir')) : array($options['app']);
        foreach ($apps as $app) {
            ...
            foreach ($envs as $env) {
                ...
                $this->logSection(
                    $this->getFullName(),
                    sprintf('Clearing cache for "%s" app and "%s" env', $app, $env)
                );
                $this->lock($app, $env);

                $event = $this->dispatcher->notifyUntil(
                    new sfEvent('cli_task.cache.clear',
