...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        if (file_exists($test)) {
            $this->logSection(
                'task',
                sprintf('A test script for the class "%s" already exists.', $r->getName()),
                null,
                'ERROR'
            );
        } else {
            $this->getFilesystem()->copy(
                $this->environment->get('sf_sift_data_dir') . '/skeleton/test/unit.php',
                $test
            );
    ...
    protected function getBootstrapPathPhp($bootstrapFile, $testFile)
    {
        if (0 === strpos($testFile, $path = realpath(dirname($bootstrapFile) . '/..'))) {
            ...
            if (0 === strpos($bootstrapFile, $this->environment->get('sf_test_dir'))
                && 0 === strpos($testFile, $this->environment->get('sf_root_dir'))
            ) {
                $path = str_repeat(
                        '/..',
                        substr_count(
                            dirname(str_replace($this->environment->get('sf_root_dir'), '', $testFile)),
                            DIRECTORY_SEPARATOR
                        )
                    ) . '/test';
            } else {
                throw new InvalidArgumentException(sprintf(
                    'A relative path from "%s" to "%s" could not be determined.',
                    $testFile,
                    $bootstrapFile
                ));
            }
    ...
    protected function getDirectories($path)
    {
        ...
        throw new InvalidArgumentException(sprintf(
            'The file "%s" is not in the project or a connected plugin’s lib directory.',
            $path
        ));
    }
