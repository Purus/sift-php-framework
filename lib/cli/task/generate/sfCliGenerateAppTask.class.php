...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        $this->getFilesystem()->replaceTokens(
            ...
            array(
                ...
                'ESCAPING_STRATEGY' => sfYamlInline::dump(
                        (boolean)sfYamlInline::parseScalar($options['escaping-strategy'])
                    ),
        ...
        $this->getFilesystem()->copy(
            $skeletonDir . '/web/index.php',
            $this->environment->get('sf_web_dir') . '/' . $indexName . '.php'
        );
        $this->getFilesystem()->copy(
            $skeletonDir . '/web/index.php',
            $this->environment->get('sf_web_dir') . '/' . $app . '_dev.php'
        );
        ...
        $this->getFilesystem()->rename(
            $appDir . '/lib/application.class.php',
            $appDir . '/lib/' . $className . '.class.php'
        );
        $this->getFilesystem()->replaceTokens(
            $appDir . '/lib/' . $className . '.class.php',
            '##',
        ...
        // fix permissions
        $fixPerms
            = new sfCliProjectPermissionsTask($this->environment, $this->dispatcher, $this->formatter, $this->logger);
