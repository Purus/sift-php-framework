...
class sfCliGeneratePluginTask extends sfCliGeneratorBaseTask
{
    ...
    protected function configure()
    {
        ...
        $this->addOptions(
            array(
                new sfCliCommandOption('module', null,
                    sfCliCommandOption::PARAMETER_REQUIRED | sfCliCommandOption::IS_ARRAY, 'Add a module'),
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        if (!preg_match('/^[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*$/', $options['test-application'])) {
            throw new sfCliCommandException(sprintf(
                'The application name "%s" is invalid.',
                $options['test-application']
            ));
        }
        ...
        // Plugin installer
        $this->getFilesystem()->rename(
            $pluginDir . '/lib/install/Install.class.php',
        ...
        if ($options['skip-test-dir']) {
            ...
            $this->getFilesystem()->mirror(
                $this->environment->get('sf_sift_data_dir') . '/skeleton/project',
                $testProject,
                $finder
            );
            $this->getFilesystem()->mirror(
                $this->environment->get('sf_sift_data_dir') . '/skeleton/app/app',
                $testApp,
                $finder
            );
            ...
            $this->getFilesystem()->rename(
                $testApp . '/lib/application.class.php',
                $testApp . '/lib/' . $className . '.class.php'
            );
