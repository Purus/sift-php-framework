...
function start_template($id, $options = array(), $type = 'text/html')
{
    ...
    if (!is_null($request->getAttribute('started', null, 'template_javascript'))) {
        throw new LogicException(sprintf(
            'Template buffering already started. Buffering template id: "%s"',
        ...
        // we will put the compiled template to cache
            ...
            if (sfConfig::get('sf_logging_enabled')) {
                sfLogger::getInstance()->info(
                    sprintf(
                        '{TemplateHelper} Adding compiled template "%s"("%s") to response.',
                        $id,
                        $cacheWebFile
                    )
                );
...
function _compile_template($buffer)
{
    ...
    if (sfConfig::get('sf_cache')) {
        ...
        if ($key !== false) {
            if (!$key) {
                $key = md5(
                    'template_javascript' . $context->getModuleName() . $context->getActionName() . $buffer . serialize(
                        $options
                    )
                );
            }
    ...
    $result = compile_javascript_template(
        $buffer,
        isset($options['compile_options']) ?
