...
function get_component($moduleName, $componentName, $vars = array(), $viewName = null)
{
    ...
    if (!$viewName) {
        ...
        if (
        $partialViewClass = sfConfig::get(sprintf('mod_' . strtolower($moduleName) . '__%s_view_class', $templateName))
        ) {
            $class = $partialViewClass;
        } elseif (
        $moduleViewClass = sfConfig::get('mod_' . strtolower($moduleName) . '_partial_view_class', $templateName)
        ) {
...
function get_partial($templateName, $variables = array(), $viewName = null)
{
    ...
    if (!$viewName) {
        ...
        require_once(sfConfigCache::getInstance()->checkConfig(
            sfConfig::get('sf_app_module_dir_name') . '/' . $moduleName . '/' . sfConfig::get(
                'sf_app_module_config_dir_name'
            ) . '/module.yml',
            true
    ...
    $view->initialize($moduleName, $actionName, '');
    $view->setPartialVars(
        true === sfConfig::get('sf_escaping_strategy') ? sfOutputEscaper::unescape($variables) : $variables
    );
...
function _call_component($moduleName, $componentName, $vars)
{
    ...
    if (!$controller->componentExists($moduleName, $componentName)) {
        ...
        throw new sfConfigurationException(sprintf(
            'The component does not exist: "%s", "%s".',
            $moduleName,
            $componentName
        ));
    }
    ...
    if (!$componentInstance->initialize($context)) {
        ...
        throw new sfInitializationException(sprintf(
            'Component initialization failed for module "%s", component "%s"',
            $moduleName,
            $componentName
        ));
    }
