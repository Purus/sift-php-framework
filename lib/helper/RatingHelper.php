...
function rating_stars($avg_rating, $options = array())
{
    ...
    if (!in_array($options['size'], array('small', 'big'))) {
        throw new InvalidArgumentException(sprintf(
            '{RatingHelper} rating_stars() "size" - "%s" option in not valid. Valid values are: "small, big"',
            $options['size']
        ));
    }
    ...
    $html = array();
    $html[] = sprintf(
        '<span class="star-rating-state%s" title="%s/%s">',
        $class ? (' ' . $class) : '',
        round($avg_rating, 1),
        $options['max']
    );
...
function rating_stars_votable($rating, $options = array())
{
    ...
    for ($i = 1; $i <= $max; $i++) {
        $html[] = sprintf(
            '<li><a id="%s%s" href="%s" title="Hodnotit %s" class="%s" rel="nofollow">%s</a></li>',
            $options['id'],
            $i,
            url_for(str_replace('%%VOTE%%', $i, $route)),
            $i,
            rating_class($i),
            $i
        );
