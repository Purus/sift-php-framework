 ...
 */
function objects_for_select($options = array(), $value_method = null, $text_method = null, $selected = null, $html_options = array())
{
...
function object_select_tag($object, $method, $options = array(), $default_value = null)
{
  ...
  // parameters
  foreach(array('order_by', 'order_type', 'with_i18n', 'hydrate_mode', 'culture') as $p)
  {
    $value = _get_option($options, $p);
    if($value)
    {
      $retrieveParams[$p] = $value;
    }
  }

  $select_options = _get_options_from_objects(sfContext::getInstance()->retrieveObjects($related_class, $peer_method, $retrieveParams), $text_method);
  ...
  if($objects)
  {
    ...
    foreach($objects as $tmp_object)
    {
      if($first)
      {
        ...
        // nested set object!
        if($tmp_object->getTable()->hasTemplate('NestedSet'))
        {
          $nested_set = true;
        }
      }
      ...
      if($nested_set)
      {
        $value = sprintf('%s%s', str_repeat('&nbsp;', $tmp_object->getLevel() * 2), $value);
      }
