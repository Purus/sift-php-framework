...
class sfMailer extends Swift_Mailer implements sfIConfigurable
{
    ...
    public function __construct(sfEventDispatcher $dispatcher, $options = array())
    {
        ...
        $this->dispatcher->notify(
            new sfEvent('mailer.pre_configure', array(
                'mailer'  => $this,
        ...
        $this->dispatcher->notify(
            new sfEvent('mailer.configure', array(
                'mailer'  => $this,
                'options' => $this->options
            ))
        );
            ...
            if (isset($transportOptions['class'])) {
                ...
                if (!class_exists($class)) {
                    throw new sfConfigurationException(sprintf(
                        'Invalid mailer transport class "%s" given for "%s" transport.',
                        $class,
                        $transportName
                    ));
                }
        ...
        switch (strtolower($type)) {
            ...
            default:
                ...
                if (!isset($transports[$type])) {
                    throw new sfConfigurationException(sprintf(
                        'The configuration specifies "%s" to use as transport, but the transport is not configured.',
                        $type
                    ));
                }
    ...
    {
        return new sfMailerMessage($this->dispatcher, $subject, $body, $contentType, $charset ? $charset
