...
class sfPHPView extends sfView
{
    ...
    protected function loadHelpers()
    {
        ...
        $helpers = array_unique(
            array_merge(sfCore::getCoreHelpers(), sfConfig::get('sf_standard_helpers', array()), $this->helpers)
        );
    ...
    protected function getVariables()
    {
        ...
        $parameters = sfCore::filterByEventListeners(
    ...
    public function configure()
    {
        ...
        require(sfConfigCache::getInstance()->checkConfig(
            sfConfig::get('sf_app_module_dir_name') . '/' . $this->moduleName . '/' . sfConfig::get(
    ...
    protected function decorate($content)
    {
        ...
        if (sfConfig::get('sf_logging_enabled')) {
            $this->getContext()->getLogger()->info(
                '{sfView} Decorate content with "{template}".',
    ...
    public function render($templateVars = array())
    {
        ...
        // decorator
        $layout = $response->getParameter(
            $this->moduleName . '_' . $this->actionName . '_layout',
            null,
            'sift/action/view'
        );
        if (false === $layout) {
        ...
        if ($retval === null) {
            ...
            if (sfConfig::get('sf_cache') && $uri !== null) {
                $viewCache->setActionCache(
                    $uri,
                    $retval,
                    $this->isDecorator() ? $this->getDecoratorDirectory() . '/' . $this->getDecoratorTemplate() : false
                );
            }
        }
