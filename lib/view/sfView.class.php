...
abstract class sfView implements sfIView
{
    ...
    public function getEscapingMethod()
    {
        ...
        if (!defined($method)) {
            throw new sfException(sprintf(
                'Escaping method "%s" is not available; perhaps another helper needs to be loaded in?',
                $method
            ));
        }
    ...
    protected function preRenderCheck()
    {
        ...
        if (!is_readable($template)) {
            ...
            throw new sfRenderException(sprintf(
                'The template "%s" does not exist in: %s',
                $template,
                $this->directory
            ));
        }
        ...
        if ($this->decorator) {
            ...
            if (!is_readable($template)) {
                ...
                throw new sfRenderException(sprintf(
                    'The decorator template "%s" does not exist or is unreadable',
                    $template
                ));
            }
        }
