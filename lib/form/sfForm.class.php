...
class sfForm implements ArrayAccess, Iterator, Countable, sfIUserAware
{
    ...
    public function __construct($defaults = array(), $options = array(), $CSRFSecret = null)
    {
        ...
        if (self::$dispatcher) {
            ...
            self::$dispatcher->notify(
                new sfEvent('form.post_configure', array(
                    'form' => $this
                ))
            );
            ...
            self::$dispatcher->notify(
                new sfEvent(sprintf(
                    'form.%s.post_configure',
                    sfInflector::tableize(get_class($this))
                ), array(
                    'form' => $this
                ))
            );
    ...
    protected function doBind(array $values)
    {
        if (self::$dispatcher) {
            ...
            $values = self::$dispatcher->filter(new sfEvent('form.filter_values', array('form' => $this)), $values)
                ->getReturnValue();
        }
        ...
        try {
            ...
            if (self::$dispatcher) {
                self::$dispatcher->notify(
                    new sfEvent('form.validation_error', array('error' => $error, 'form' => $this))
                );
            }
     ...
     */
    public function embedFormForEach(
        $name,
        sfForm $form,
        $n,
        $decorator = null,
        ...
        $decorator = null === $decorator ? $widgetSchema->getFormFormatter()->getDecoratorFormat() : $decorator;
        $innerDecorator
            = null === $innerDecorator ? $widgetSchema->getFormFormatter()->getDecoratorFormat() : $innerDecorator;

        $this->widgetSchema[$name]
            = new sfWidgetFormSchemaDecorator(new sfWidgetFormSchemaForEach(new sfWidgetFormSchemaDecorator($widgetSchema, $innerDecorator), $n, $options, $attributes), $decorator);
    ...
    public function mergeForm(sfForm $form)
    {
        ...
        $this->getWidgetSchema()->setLabels(
            $form->getWidgetSchema()->getLabels() + $this->getWidgetSchema()->getLabels()
        );
    ...
    public function setDefaults($defaults)
    {
        ...
        if ($this->isCSRFProtected()) {
            $this->setDefault(
                self::$CSRFFieldName,
                $this->getCSRFToken($this->localCSRFSecret ? $this->localCSRFSecret : self::$CSRFSecret)
            );
        }
    ...
    public function renderFormTag($url, array $attributes = array())
    {
        ...
        if (!in_array($attributes['method'], array('get', 'post'))) {
            $html = $this->getWidgetSchema()->renderTag(
                'input',
                array('type' => 'hidden', 'name' => 'sf_method', 'value' => $attributes['method'], 'id' => false)
            );
    ...
    public function offsetGet($name)
    {
        if (!isset($this->formFields[$name])) {
            ...
            $this->formFields[$name] = new $class($this, $widget, $this->getFormFieldSchema(
            ), $name, $value, $this->errorSchema[$name]);
            if (isset($this->validatorSchema[$name])) {
    ...
    public function getFormFieldSchema()
    {
        if (null === $this->formFieldSchema) {
            ...
            $this->formFieldSchema
                = new sfFormFieldSchema($this, $this->widgetSchema, null, null, $values, $this->errorSchema);
        }
