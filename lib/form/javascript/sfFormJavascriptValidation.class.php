...
class sfFormJavascriptValidation
{
    ...
    public static function getForForm(
        ...
        if ($diff = array_diff($optionKeys, array_merge($currentOptionKeys, self::$validOptions))) {
            throw new InvalidArgumentException(sprintf(
                'sfFormJavascriptValidation does not support the following options: \'%s\'.',
                implode('\', \'', $diff)
            ));
        }
        ...
        $js[] = sprintf(
            'Application.behaviors.setup%sFormValidation = function(context)',
            sfInflector::camelize(str_replace('-', '_', strtolower($formId)))
        );
        $js[] = '{';
        ...
        $js[] = sprintf(
            '  $(\'%s\', $(\'#%s\')).attr(\'generated\', true)%s;',
            $options['error_element'],
            $formId,
     ...
     */
    public static function getValidationRulesAndMessagesForForm(
        sfForm $form,
        $embededFormName = null,
        sfForm $parentForm = null
    ) {
        ...
        foreach ($form->getValidatorSchema()->getFields() as $field_name => $validator) {
            ...
            if ($validator instanceof sfValidatorSchemaForEach || $validator instanceof sfValidatorSchema) {
                $skipToNext = true;
            }
        ...
        foreach ($embeded as $embededFormName => $embededForm) {
            ...
            if (get_class($embededForm) == 'sfForm') {
                ...
                foreach ($embededForms as $name => $embededForm2) {
                    ...
                    list($r, $m) = self::getValidationRulesAndMessagesForForm(
                        $embededForm2,
                        $embededFormName,
        ...
        foreach ($messages as $field_name => $_field_messages) {
            foreach ($_field_messages as $index => $message) {
                if ($message->hasParameters()) {
                    $msg = new sfJsonExpression(sprintf(
                        "function(parameters, element) { return '%s'.replace('%%value%%', jQuery(element).val(), 'g'); }",
