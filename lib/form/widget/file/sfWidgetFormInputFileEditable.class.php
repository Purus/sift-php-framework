...
class sfWidgetFormInputFileEditable extends sfWidgetFormInputFile
{
    ...
    public function render($name, $value = null, $attributes = array(), $errors = array())
    {
        ...
        if ($this->getOption('with_delete')) {
            ...
            $delete = $this->renderTag(
                'input',
                array_merge(array('type' => 'checkbox', 'name' => $deleteName, 'value' => '1'), $attributes)
            );
            $deleteLabel = $this->translate($this->getOption('delete_label'));
            $deleteLabel = $this->renderContentTag(
                'label',
                $deleteLabel,
                array_merge(array('for' => $this->generateId($deleteName), 'class' => 'inline'))
            );
        } else {
            $delete = '';
            $deleteLabel = '';
    ...
    protected function getFileAsTag($attributes)
    {
        if ($this->getOption('is_image')) {
            return false !== $this->getOption('file_src') ? $this->renderTag(
                'img',
