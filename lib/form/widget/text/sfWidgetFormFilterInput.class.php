...
class sfWidgetFormFilterInput extends sfWidgetForm
{
    ...
    protected function configure($options = array(), $attributes = array())
    {
        ...
        $this->addOption(
            'template',
            sprintf('%%input%% %s %%empty_checkbox%% %%empty_label%%', sfWidget::isXhtml() ? '<br />' : '<br>')
        );
    }
    ...
    public function render($name, $value = null, $attributes = array(), $errors = array())
    {
        ...
        return strtr(
            ...
            array(
                ...
                '%empty_checkbox%' => $this->getOption('with_empty') ? $this->renderTag(
                        'input',
                        array(
                            'type'    => 'checkbox',
                            'name'    => $name . '[is_empty]',
                            'checked' => $values['is_empty'] ? 'checked' : ''
                        )
                    ) : '',
                '%empty_label%'    => $this->getOption('with_empty') ? $this->renderContentTag(
                        'label',
                        $this->translate($this->getOption('empty_label')),
                        array('for' => $this->generateId($name . '[is_empty]'))
                    ) : '',
            )
        );
    }
