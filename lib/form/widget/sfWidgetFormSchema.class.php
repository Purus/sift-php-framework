...
class sfWidgetFormSchema extends sfWidgetForm implements ArrayAccess
{
    ...
    /**
     * Sets the generic default formatter name used by the class.
     *
    ...
    {
        return
            null === $this->options['form_formatter'] ? self::$defaultFormatterName : $this->options['form_formatter'];
    }
     ...
     *
     * Do not use a reserved word in the name format. If you do, Sift may act in an unexpected manner.
     *
     * For Sift, the following words are reserved and must NOT be used as
     * the name format:
     *
        ...
        if (2 == func_num_args()) {
            if (!isset($this->fields[$name])) {
                throw new InvalidArgumentException(sprintf(
                    'Unable to set the label on an unexistant widget ("%s").',
                    $name
                ));
            }
    ...
    public function getLabel($name = null)
    {
        if (1 == func_num_args()) {
            if (!isset($this->fields[$name])) {
                throw new InvalidArgumentException(sprintf(
                    'Unable to get the label on an unexistant widget ("%s").',
                    $name
                ));
            }
    ...
    public function setHelp($name, $value = null)
    {
        if (2 == func_num_args()) {
            if (!isset($this->fields[$name])) {
                throw new InvalidArgumentException(sprintf(
                    'Unable to set the help on an unexistant widget ("%s").',
                    $name
                ));
            }
    ...
    public function getHelp($name = null)
    {
        if ($name !== null) {
            if (!isset($this->fields[$name])) {
                throw new InvalidArgumentException(sprintf(
                    'Unable to get the help on an unexistant widget ("%s").',
                    $name
                ));
            }
    ...
    public function renderField($name, $value = null, $attributes = array(), $errors = array())
    {
        ...
        return $clone->render(
            ...
            $value,
            array_merge($clone->getAttributes(), $attributes),
            $errors,
            $widget
        );
    }
    ...
    public function render($name, $values = array(), $attributes = array(), $errors = array())
    {
        ...
        /* @var $formFormat sfWidgetFormSchemaFormatter */
        $errorCssClass = $formFormat->getErrorCssClass();
        ...
        foreach ($this->positions as $name) {
            ...
            if ($widget instanceof sfWidgetForm && $widget->isHidden()) {
                    ...
                    $attributes['class'] = trim(
                        implode(
                            ...
                            array_merge(
                                explode(
                                    ' ',
                                    ($this->parent ? $this->parent->getAttribute('class') : '')
                                ),
                                $classes
                            )
                        )
                ...
                $label = ($widget instanceof sfWidgetFormSchema || !$widget->isLabelable()) ? $this->getFormFormatter()
                    ->generateLabelName($name) : $this->getFormFormatter()->generateLabel($name);
        ...
        if ($rows) {
            ...
            for ($i = 0, $max = count($rows); $i < $max; $i++) {
                $rows[$i] = strtr(
                    $rows[$i],
                    array('%hidden_fields%' => $i == $max - 1 ? implode("\n", $hiddenRows) : '')
                );
            }
        } else {
            // only hidden fields
    ...
    public function setPositions(array $positions)
    {
        ...
        if ($diff = array_diff($positions, $current)) {
            throw new InvalidArgumentException('Widget schema does not include the following field(s): ' . implode(
                ', ',
                $diff
            ));
        }
        ...
        if ($diff = array_diff($current, $positions)) {
            throw new InvalidArgumentException('Positions array must include all fields. Missing: ' . implode(
                ', ',
                $diff
            ));
        }
    ...
    public function renderForJavascriptTemplate()
    {
        return str_replace(
            ...
            $this->render(
                'TEMPLATENAMEPLACEHOLDER',
                array('TEMPLATEVALUEPLACEHOLDER'),
                array(
