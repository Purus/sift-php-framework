...
abstract class sfWidget
{
    ...
    public function renderTag($tag, $attributes = array())
    {
        ...
        return sprintf(
            '<%s%s%s',
            $tag,
            $this->attributesToHtml($attributes),
    ...
    protected function encloseInTag($value, $attributes, $tag)
    {
        ...
        $value = '<' . $tag .
            implode(
                '',
                array_map(
                    array($this, 'attributesToHtmlCallback'),
                    array_keys($spanAttributes),
                    array_values($spanAttributes)
                )
            ) .
            '>' .
            $value .
    ...
    public function attributesToHtml($attributes)
    {
        ...
        return implode(
            '',
            array_map(array($this, 'attributesToHtmlCallback'), array_keys($attributes), array_values($attributes))
        );
    }
    ...
    protected function attributesToHtmlCallback($k, $v)
    {
        ...
        return false === $v || null === $v || ('' === $v && 'value' != $k)
            ? ''
            : sprintf(
                ' %s="%s"',
                $k,
