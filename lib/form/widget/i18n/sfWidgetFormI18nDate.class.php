...
class sfWidgetFormI18nDate extends sfWidgetFormDate
{
  ...
  protected function getMonthFormat($culture, $monthFormat)
  {
    ...
    {
      case 'name':
        return array_combine(range(1, 12), sfI18nDateTimeFormat::getInstance($culture)->getMonthNames());
      case 'short_name':
        return array_combine(range(1, 12), sfI18nDateTimeFormat::getInstance($culture)->getAbbreviatedMonthNames());
      case 'number':
  ...
  {
    $dateFormat = sfI18nDateTimeFormat::getInstance($culture)->getShortDatePattern();
