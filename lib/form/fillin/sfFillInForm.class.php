...
class sfFillInForm {
  ...
  public function fillInXhtml($xml, $formName, $formId, $values)
  {
    ...
    $prolog_regexp = '/^' . preg_quote(sprintf('<?xml version="1.0" encoding="%s"?>', strtoupper(sfConfig::get('sf_charset', 'UTF-8')))) . '\s*/';
  ...
  public function fillInXml($xml, $formName, $formId, $values)
  {
    ...
    $prolog = sprintf('<?xml version="1.0" encoding="%s"?>', strtoupper(sfConfig::get('sf_charset', 'UTF-8')));
    if(strpos($xml, $prolog) === false)
    {
      $xml = $prolog . $xml;
    }
  ...
  public function fillInDom($dom, $formName, $formId, $values)
  {
    ...
    foreach($xpath->query($query, $form) as $element)
    {
      ...
      else if($element->nodeName == 'select')
      {
        ...
        {
          $value = $this->getValue($values, $name, true);
        }
