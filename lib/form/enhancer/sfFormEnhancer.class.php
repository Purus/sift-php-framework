...
class sfFormEnhancer extends sfConfigurable implements sfIFormEnhancer
{
    ...
    public static function factory($class, $options = array())
    {
        ...
        if (!$enhancer instanceof sfIFormEnhancer) {
            throw new InvalidArgumentException(sprintf(
                'Form enhancer "%s" does not implement sfIFormEnhancer interface.',
                $class
            ));
        }
    ...
    public function filterTemplateVariables(sfEvent $event, array $variables)
    {
        foreach ($variables as $variable) {
            if ($variable instanceof sfForm && !in_array($variable, $this->enhanced, true)) {
     ...
     */
    protected function enhanceFormFields(
        sfForm $form,
        sfFormFieldSchema $fieldSchema,
        $formClass,
        array $embeddedForms = array()
    ) {
            ...
            if ($field instanceof sfFormFieldSchema) {
                if (isset($embeddedForms[$field->getName()])) {
                    ...
                    $this->enhanceFormFields(
                        $embededForm,
                        $form->getFormFieldSchema(),
                        get_class($embededForm),
                        $embededForm->getEmbeddedForms()
                    );
                } else {
        ...
        foreach (self::getLineage($formClass) as $class) {
            foreach ($this->getOption(sprintf('forms.%s', $class), array()) as $name => $params) {
                ...
                if (!isset($fieldSchema[$name])) {
                    throw new InvalidArgumentException(sprintf(
                        'Invalid field "%s". Cannot enhance the form "%s".',
                        $name,
                        $formClass
                    ));
                }
                        ...
                        if ('class' == $name) {
                            ...
                            $widget->setAttribute(
                                ...
                                trim(
                                    implode(
                                        ' ',
                                        array_merge(explode(' ', $widget->getAttribute('class')), array($value))
                                    )
                                )
                            );
                        } else {
            ...
            if (is_array($config)) {
                ...
                foreach ($config['attributes'] as $name => $value) {
                    ...
                    if ('class' == $name) {
                        ...
                        $widget->setAttribute(
                            $name,
                            implode(' ', array_merge(explode(' ', $widget->getAttribute('class')), array($value)))
                        );
                    } else {
                        $widget->setAttribute($name, $value);
                    }
        ...
        if (preg_match('/\!\!callback:\s?(.*+)/', $value, $matches)) {
            ...
            if ($callback && !is_callable($callback, false, $callableName)) {
                throw new sfConfigurationException(sprintf(
                    'Invalid callback "%s" given for "%s". Key: "%s"',
                    $callableName,
                    get_class($widget),
                    $name
                ));
