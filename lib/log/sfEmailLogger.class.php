    ...
    public function setup()
    {
        ...
        if (!is_readable($this->getOption('template'))) {
            throw new sfConfigurationException(sprintf(
                'The template "%s" is not readable or does not exist',
        ...
        if (!$this->getOption('sender_email')) {
            $this->setOption(
                'sender_email',
                sprintf('webmaster@%s', isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'localhost')
            );
        }
    ...
    protected function getRequestDumpAsArray()
    {
        ...
        foreach (
            ...
            if (isset($values['cookie'][$sessionName])) {
                $values['cookie'][$sessionName]
                    = substr($values['cookie'][$sessionName], 0, 3) . '-the-rest-removed-for-security-reasons';
            }
        }
