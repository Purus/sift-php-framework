...
class sfLogAnalyzer extends sfConfigurable implements Countable
{
    ...
    protected function doProcess()
    {
        ...
        $pattern = sprintf(
            '/%s/',
            strtr(
                preg_quote($this->getOption('line_format'), '/'),
                array(
        ...
        while (!feof($handle)) {
            ...
            $stmt = $this->dbh->prepare(
                'INSERT INTO log_analyzed (id, type, level, message, extra, created_at) VALUES (?, ?, ?, ?, ?, ?)'
            );
            $stmt->execute(
                array(
                    $i,
                    ...
                    $matches['extra'] ? $matches['extra'] : null,
                    date('Y-m-d H:i:s', strtotime($matches['time']))
                )
            );
        }
    ...
    public function getLevels()
    {
        $this->process();
        $stmt = $this->dbh->query(
            'SELECT COUNT(id) AS count, level AS level FROM log_analyzed GROUP BY level ORDER BY count DESC'
        );
    ...
    public function getTypes()
    {
        $this->process();
        $stmt = $this->dbh->query(
            'SELECT COUNT(id) AS count, type AS type FROM log_analyzed GROUP BY type ORDER BY count DESC'
        );
    ...
    {
        $query = sprintf(
            'SELECT * FROM log_analyzed%sORDER BY created_at DESC',
