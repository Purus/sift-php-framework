...
class sfImageThumbnailGeneric extends sfImageTransformAbstract
{
    ...
    public function __construct($width, $height, $method = 'fit', $background = null)
    {
        if (!$this->setWidth($width) || !$this->setHeight($height)) {
            throw new sfImageTransformException(sprintf(
                'Cannot perform thumbnail, a valid width and height must be supplied'
            ));
        }
    ...
    public function setMethod($method)
    {
        ...
        if ($key = array_search($method, $map)) {
            $message = sprintf(
                'sfImageTransformPlugin thumbnail method \'%s\' is depreciated use \'%s\'',
                $method,
                $key
            );
            sfContext::getInstance()->getEventDispatcher()->notify(
                new sfEvent($this, 'application.log', array($message, 'priority' => sfLogger::ERR))
            );
        }
