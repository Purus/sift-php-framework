...
class sfImage implements sfIDataUriConvertable
{
    ...
    public function load($filename, $mime = '', $fixOrientation = true)
    {
        if (file_exists($filename) && is_readable($filename)) {
            ...
            if ('' == $mime) {
                throw new sfImageTransformException(sprintf(
                    'Mime type of the file "%s" not specified nor detected.',
                    $filename
                ));
            }
        ...
        throw new sfImageTransformException(sprintf(
            'Unable to load %s. File does not exist or is unreadable',
            $filename
        ));
    }
    ...
    public function __call($name, $arguments)
    {
        ...
        elseif (class_exists($class_generic, true)) {
            ...
            throw new sfImageTransformException(sprintf(
                'Unsupported transform %s. Cannot find %s adapter or generic transform class',
                $name,
                $this->getAdapter()->getAdapterName()
            ));
        }
