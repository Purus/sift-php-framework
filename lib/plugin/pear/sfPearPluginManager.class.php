...
class sfPearPluginManager extends sfConfigurable
{
    ...
    public function getInstalledPlugins($skipSift = true)
    {
        ...
        foreach ($this->environment->getRegistry()->packageInfo(null, null, null) as $channel => $packages) {
            foreach ($packages as $package) {
                $_installed = $this->environment->getRegistry()->getPackage(
                    isset($package['package']) ? $package['package'] : $package['name'],
                    $channel
                );
    ...
    protected function doInstallPlugin($plugin, $options = array(), $force = false)
    {
        $channel = isset($options['channel'])
            ? $options['channel']
            : $this->environment->getConfig()->get(
                'default_channel'
            );
        $stability = isset($options['stability'])
            ? $options['stability']
            : $this->environment->getConfig()->get(
                'preferred_state',
                null,
        ...
        $this->dispatcher->notify(
            new sfEvent('plugin.pre_install',
                array(
                    'manager'    => $this,
                    'channel'    => $channel,
                    'plugin'     => $plugin,
        ...
        if ($isPackage) {
            ...
            if (!$version) {
                ...
                if (!$this->isPluginCompatible($plugin, $version)) {
                    throw new sfPluginDependencyException(sprintf(
            ...
            if (@PEAR::isError($download)) {
                throw new sfPluginException(sprintf(
                    'Problem downloading the plugin "%s": %s',
                    $plugin,
                    $download->getMessage()
                ));
            }
        ...
        if (count($errors)) {
            ...
            if (!count($downloaded)) {
                throw new sfPluginException(sprintf(
                    'Plugin "%s" installation failed. %s',
                    $plugin,
                    implode("\n", $err)
                ));
            }
        ...
        if (@PEAR::isError($info) || !is_array($info)) {
            $this->dispatcher->notify(
                new sfEvent('plugin.post_install.failure', array(
                    'channel' => $channel,
                    'manager' => $this,
            ...
            throw new sfPluginException(sprintf(
                'Installation of "%s" plugin failed: %s',
                $plugin,
                $info->getMessage()
            ));
        }
        ...
        $this->dispatcher->notify(
            new sfEvent('plugin.post_install.success', array(
                'channel' => $channel,
                'manager' => $this,
                'plugin'  => $pluginPackage->getPackage()
            ))
    ...
    public function uninstallPlugin($plugin, $channel = null)
    {
        ...
        $this->dispatcher->notify(
            new sfEvent('plugin.pre_uninstall',
                array(
                    'channel' => $channel,
                    'manager' => $this,
                    'version' => $existing,
        ...
        if ($ret) {
            $this->dispatcher->notify(
                new sfEvent('plugin.post_uninstall.success', array(
                    'channel' => $channel,
                    'manager' => $this,
                    'version' => $existing,
                    'plugin'  => $plugin
            ...
            $this->dispatcher->notify(
                new sfEvent('plugin.post_uninstall.failure', array(
                    'channel' => $channel,
                    'manager' => $this,
                    'version' => $existing,
                    'plugin'  => $plugin
    ...
    public function checkPluginDependencies($plugin, $version, $options = false)
    {
        ...
        foreach ($deps as $dependency) {
            if (!$this->checkDependency($dependency)) {
                $version = (isset($dependency['min']) ? ' >= ' . $dependency['min'] : '') . (isset($dependency['max']) ?
                        ' <= ' . $dependency['max'] : '') . (isset($dependency['exclude']) ?
                ...
                if (isset($options['install-deps']) && $options['install-deps']) {
                    try {
                        $this->doInstallPlugin(
                            $dependency['name'],
                            array_merge($options, array('channel' => $dependency['channel']))
                        );
                    } catch (sfException $e) {
                        throw new sfPluginRecursiveDependencyException(sprintf(
                ...
                throw new sfPluginDependencyException(sprintf(
                    'Unable to install plugin "%s" (version %s) because it depends on plugin "%s" which is not installed (install dependencies by hand or use the --install-deps option for automatic installation).',
                    $plugin,
                    $version,
                    $dependency['name']
                ));
    ...
    {
        $dependencyChecker = new PEAR_Dependency2($this->environment->getConfig(), array(), array(
            'package' => '',
            'channel' => ''
        ));
