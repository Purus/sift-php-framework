...
/**
 * sfPearRestPlugin interacts with a sift plugin channel.
 *
 * @package    Sift
...
class sfPearRestPlugin extends sfPearRest11
{
    ...
    protected function getRESTBase($channelName)
    {
        ...
        if (!$channel->supportsREST($mirror)) {
            throw new sfPluginRestException(sprintf(
    ...
    public function getPluginVersions($plugin, $stability = null)
    {
        ...
        if (PEAR::isError($allreleases)) {
            throw new sfPluginRestException(sprintf(
                'Unable to get information for plugin "%s": %s',
                $plugin,
                $allreleases->getMessage()
            ));
        }
    ...
    {
        $dependencies = $this->_rest->retrieveData(
            $this->restBase . 'r/' . strtolower($plugin) . '/deps.' . $version . '.txt'
        );
        if (PEAR::isError($dependencies)) {
            throw new sfPluginRestException(sprintf(
                'Unable to get dependencies information for plugin "%s": %s',
                $plugin,
                $dependencies->getMessage()
            ));
        }
    ...
    public function getPluginDownloadURL($plugin, $version, $stability)
    {
        ...
        $info = $this->getDownloadURL(
            $this->restBase,
            array('channel' => $this->channel, 'package' => $plugin, 'version' => $version),
            $stability,
            $installed
        );
        if (PEAR::isError($info)) {
