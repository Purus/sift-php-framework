...
class sfUser extends sfConfigurable implements sfIUser, sfIService, ArrayAccess
{
    ...
    public function setup()
    {
        ...
        if ($this->getOption('timezone_enabled')) {
            ...
            elseif ($offset !== null
                && ($zone = sfDateTimeZone::getNameFromOffset($offset, (boolean)$daylightsavings))
            ) {
            ...
            if ($names) {
                ...
                if (sfConfig::get('sf_logging_enabled')) {
                    sfLogger::getInstance()->info(
                        '{sfUser} Flagged old flash messages ("' . implode('", "', $names) . '")'
                    );
                }
            }
        }
    ...
    public function setCulture($culture)
    {
        ...
        if ($this->culture && ($this->culture !== $culture)) {
            $this->dispatcher->notify(
                new sfEvent('user.change_culture', array(
        ...
        sfConfig::set(
            'sf_routing_defaults',
            array_merge((array)sfConfig::get('sf_routing_defaults'), array('sf_culture' => $culture))
        );
    }
    ...
    public function setTimezone($timezone)
    {
        ...
        $this->dispatcher->notify(
            new sfEvent('user.change_timezone', array(
                    'user'     => $this,
                    'timezone' => $timezone,
                    'old'      => $old
                )
    ...
    public function shutdown()
    {
        ...
        if ($this->getOption('use_flash')
            && ($names = $this->attributeHolder->getNames(sfUser::FLASH_NAMESPACE . '/remove'))
        ) {
            foreach ($names as $name) {
            ...
            if (sfConfig::get('sf_logging_enabled')) {
                sfLogger::getInstance()->info(
                    '{sfFilter} Removed old flash messages ("' . implode('", "', $names) . '")'
                );
            }
        }
