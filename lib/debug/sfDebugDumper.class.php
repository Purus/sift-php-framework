...
class sfDebugDumper
{
    ...
    private static function toHtml($var, array $options = null, $includeAssets = true)
    {
        ...
        return self::render(
            ...
            array(
                ...
                'js'            => $js,
                'title'         =>
                    $code . ($file ? htmlspecialchars(sprintf(' in file %s on line %s', $file, $line), ENT_QUOTES)
                        : ''),
                'file_edit_url' => self::getFileEditUrl($file, $line),
                'location'      => $file ? sprintf(
                        '<a href="%s">in file %s (%s)</a>',
                        htmlspecialchars(self::getFileEditUrl($file, $line)),
                        self::shortenFilePath($file),
                        $line
                    ) : '',
                'label'         => $code
            )
    ...
    {
        return strtr(
            self::$defaultOptions['file_url_format'],
            array(
                '%file%' => $file,
                '%line%' => $line
            )
        ...
        return htmlspecialchars_decode(
            strip_tags(
                preg_replace_callback(
                    '#<span class="debug-dump-(\w+)">|</span>#',
                    array('sfDebugDumper', 'terminalReplacementCallback'),
                    self::toHtml($var, $options, false)
                )
            ),
            ENT_QUOTES
        );
    ...
    {
        return "\033[" . (isset($match[1], sfDebugDumper::$terminalColors[$match[1]])
            ? sfDebugDumper::$terminalColors[$match[1]] : '0') . "m";
    }
    ...
    {
        return sprintf(
            '<span class="number double">%s%s</span>',
            var_export($var, true),
            (strpos($var, '.') === false ? '.0' : '')
        ) . self::EOL;
    }
    ...
    {
        return sprintf(
            '<span class="debug-dump-string">%s</span>%s',
            self::encodeString(
                $options['truncate'] && strlen($var) > $options['truncate'] ?
                    substr($var, 0, $options['truncate']) . ' ... ' : $var
            ),
    ...
    private static function dumpArray(&$var, $options, $level)
    {
        ...
        if (empty($var)) {
            ...
            $out
                = '<a href="#" class="debug-dump-toggler level-' . $level . (!$collapsed ? ' opened' : '') . '">' . $out
                . count($var) . ")</a>\n<div class=\"debug-dump-collapsable level-" . $level . ($collapsed
                    ? ' collapsed' : '') . "\">";
            $var[$marker] = true;
            foreach ($var as $k => & $v) {
                if ($k !== $marker) {
                    $out .= '<span class="debug-dump-indent">   ' . str_repeat('|  ', $level) . '</span>'
                        . '<span class="debug-dump-key">' . (preg_match('#^\w+\z#', $k) ? $k : self::encodeString($k))
                        . '</span> => '
    ...
    private static function dumpObject(&$var, $options, $level)
    {
        ...
        $out = '<span class="debug-dump-object">' . get_class($var) . '</span> <span class="debug-dump-hash">(#'
            . spl_object_hash($var) . ')</span>';
    ...
    private static function dumpResource(&$var, $options, $level)
    {
        ...
        if (isset(self::$resources[$type])) {
            ...
            foreach (call_user_func(self::$resources[$type], $var) as $k => $v) {
                $out .= '<span class="debug-dump-indent">   ' . str_repeat('|  ', $level) . '</span>'
                    . '<span class="debug-dump-key">' . htmlspecialchars($k) . "</span> => " . self::dumpVar(
                        $v,
                        $options,
                        $level + 1
                    );
