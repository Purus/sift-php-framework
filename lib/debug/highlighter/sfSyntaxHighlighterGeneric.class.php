...
class sfSyntaxHighlighterGeneric extends sfSyntaxHighlighter
{
  ...
  {
    $this->addPattern('/(&gt;=|&amp;|&lt;=|&gt;|&lt;(?![\?])|=(?![^<>]*>)|\+|\-|\*|[:]{2}|[\|]{2}|[&]{2}|\!)/', '<span class="' . $this->getCssPrefix() . 'keyword">$1</span>');
  }
  ...
  {
    $this->addPattern('/(?<!\w)(0x[\da-f]+|\d+)(?!\w)/ix', '<span class="' . $this->getCssPrefix() . 'int">$1</span>');
  }
  ...
  {
    $this->addPattern('/(?<!\w|>|\$)([A-Z_0-9]{2,})(?!\w|\[)/x', '<span class="' . $this->getCssPrefix() . 'int">$1</span>');
  }
  ...
  {
    $this->addPattern('/(?<!\w|\$|\%|\@|>|\\\)(and|or|xor|for|do|while|foreach|as|return|die|exit|if|then|else|
            ...
            char|signed|unsigned|short|long)(?!\w|=")/ix', '<span class="' . $this->getCssPrefix() . 'keyword">$1</span>');
  }
  ...
  {
    $this->addPattern('/(?<!\w|\$|\%|\@|>)(true|false|null)(?!\w|=")/ix', '<span class="' . $this->getCssPrefix() . 'int">$1</span>');
  }
  ...
  {
    $this->addPattern('/(?<!\w)((\$|\%|\@)(\-&gt;|\w)+)(?!\w)/ix', '<span class="' . $this->getCssPrefix() . 'variable">$1</span>');
  }
