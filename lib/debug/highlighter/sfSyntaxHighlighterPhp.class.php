...
class sfSyntaxHighlighterPhp extends sfSyntaxHighlighterGeneric
{
    ...
    protected function setup()
    {
        ...
        $this->addPattern(
            '/(?<!\w)(0x[\da-f]+|\d+)(?!\w)/ix',
            '<span class="' . $this->getCssPrefix() . 'number">$1</span>'
        );
        $this->addVarPattern();
        ...
        $this->addPattern(
            '/(class|namespace)\s([^\s|\;]+)/',
            '$1 <span class="' . $this->getCssPrefix() . 'method">$2</span>'
        );
        $this->addPattern(
            '/(include(_once)?|namespace|require(_once)?)([(\s|\()])/',
            '<span class="' . $this->getCssPrefix() . 'keyword">$1</span>$4'
        );
        $this->addPattern(
            '/new\s([^()\$]+)/',
            '<span class="' . $this->getCssPrefix() . 'keyword">new</span> <span class="' . $this->getCssPrefix()
            . 'function">$1</span>'
        );

        $this->addPattern(
            '/extends\s([^\s]+)/',
            'extends <span class="' . $this->getCssPrefix() . 'method"><em>$1</em></span>'
        );
        $this->addPattern(
            '/([\:]{2})([A-Z_0-9]+)(;|,|:|\))/',
            '::<span class="' . $this->getCssPrefix() . 'constant">$2</span>$3'
        );
        $this->addPattern(
            '/([\\\a-zA-Z_0-9]+)([\:]{2})/',
            '<span class="' . $this->getCssPrefix() . 'class">$1</span><span class="' . $this->getCssPrefix()
            . 'keyword">::</span>'
        );
        ...
        $this->addPattern(
            '/function\s(.+)\(/',
            'function <span class="' . $this->getCssPrefix() . 'method">$1</span>('
        );
        $this->addPattern('/([^_])array\(/', '$1<span class="' . $this->getCssPrefix() . 'function">array</span>(');
        $this->addPattern(
            '/(\()([a-zA-Z0-9]+)(\s)(\$)/',
            '$1<span class="' . $this->getCssPrefix() . 'function">$2</span> $'
        );

        $this->addPattern(
            '/instanceof\s(.+)\)/',
            '<span class="' . $this->getCssPrefix() . 'keyword">instanceof</span> <span class="' . $this->getCssPrefix()
            . 'function">$1</span>)'
        );
        ...
        $this->addPattern(
            '/use\s(.*);/',
            '<span class="' . $this->getCssPrefix() . 'keyword">use</span> <span class="' . $this->getCssPrefix()
            . 'function">$1</span>;'
        );
