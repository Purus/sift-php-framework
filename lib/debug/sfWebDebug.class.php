...
class sfWebDebug extends sfConfigurable
{
    ...
    public function __construct(sfVarLogger $logger, sfEventDispatcher $dispatcher, array $options = array())
    {
        ...
        $this->dispatcher->notify(
            new sfEvent('web_debug.load_panels', array(
                'web_debug' => $this
            ))
        );
        ...
        foreach (array_keys($this->getOption('panels')) as $panel) {
            ...
            if (!isset($options['class'])) {
                throw new sfConfigurationException(sprintf(
                    'The debug panel "%s" configuration is missing the class option.',
                    $panel
                ));
            }
    ...
    public function getHtml()
    {
        ...
        return $this->render(
            sfConfig::get('sf_sift_data_dir') . '/web_debug/web_debug.php',
            array(
    ...
    {
        return array_merge(
            (array)$this->getOption(sprintf('panels.%s', $panel), array()),
            array(
    ...
    public function decorateCachedContent(sfEvent $event, $content)
    {
        ...
        return $this->render(
            sfConfig::get('sf_sift_data_dir') . '/web_debug/cache/_cache_fragment.php',
            array(
