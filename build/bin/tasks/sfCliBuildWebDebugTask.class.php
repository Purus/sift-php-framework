...
EOF;
    ...
    protected function buildIcons()
    {
        ...
        $content = preg_replace(
            '/protected static \$icons = array *\(.*?\);/s',
            sprintf("protected static \$icons = array(\n%s  );", $php),
            file_get_contents($iconClassFile)
        );
    ...
    protected function buildDebug()
    {
        ...
        $logo = new sfDataUri('image/png', file_get_contents(
            $sourceDir . '/images/logo.png'
        ), sfDataUri::ENCODING_BASE64);
        $loader = new sfDataUri('image/gif', file_get_contents(
            $sourceDir . '/images/loader.gif'
        ), sfDataUri::ENCODING_BASE64);
        $toolbar = new sfDataUri('image/gif', file_get_contents(
            $sourceDir . '/images/toolbar.gif'
        ), sfDataUri::ENCODING_BASE64);

        $this->getFilesystem()->replaceTokens(
            sfFinder::type('file')->in($tmp),
            '//~//',
            '//~//',
            array(
