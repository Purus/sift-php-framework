...
EOF;
  ...
  protected function buildPhoneData()
  {
    ...
    foreach($countries as $countryCode)
    {
      ...
      $numberFormats = $territory->xpath(sprintf('//territory[@id="%s"]/availableFormats/numberFormat', $countryCode));
  ...
  protected function buildCldrData($buildCultures)
  {
    ...
    foreach($files as $file)
    {
      ...
      foreach($territories as $_territory)
      {
        ...
        if(preg_match("/^\d+$/", $territoryType) || $territoryType == 'ZZ' || (string) $a->draft == 'unconfirmed')
        {
          continue;
        }
