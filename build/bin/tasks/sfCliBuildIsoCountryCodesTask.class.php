...
EOF;
    ...
    protected function buildCountryCodes()
    {
        ...
        $countriesAlpha2Code
            = $countriesAlpha3Code = $alpha2Alpha3Map = $alpha3Alpha2Map = $euCountriesAlpha3 = array();
        ...
        $content = preg_replace(
            '/protected static \$countriesAlpha2 = array *\(.*?\);/s',
            sprintf("protected static \$countriesAlpha2 = array(\n%s  );", $alpha2),
            $fileContents
        );

        $content = preg_replace(
            '/protected static \$countriesAlpha3 = array *\(.*?\);/s',
            sprintf("protected static \$countriesAlpha3 = array(\n%s  );", $alpha3),
            $content
        );

        $content = preg_replace(
            '/protected static \$alpha2ToAlpha3Map = array *\(.*?\);/s',
            sprintf("protected static \$alpha2ToAlpha3Map = array(\n%s  );", $alpha2ToAlpha3),
            $content
        );

        $content = preg_replace(
            '/protected static \$alpha3ToAlpha2Map = array *\(.*?\);/s',
            sprintf("protected static \$alpha3ToAlpha2Map = array(\n%s  );", $alpha3ToAlpha2),
            $content
        );

        $content = preg_replace(
            '/protected static \$euCountriesAlpha2 = array *\(.*?\);/s',
            sprintf("protected static \$euCountriesAlpha2 = array(\n%s  );", $euAlpha2),
            $content
        );

        $content = preg_replace(
            '/protected static \$euCountriesAlpha3 = array *\(.*?\);/s',
